...
| 3           | User config dirs: `~/.san/agent/lsp.*`, `~/.claude/lsp.*`, `~/.codex/lsp.*`, `~/.gemini/lsp.*`                              |
| 2           | Project config dirs: `<project>/.san/lsp.*`, `<project>/.claude/lsp.*`, `<project>/.codex/lsp.*`, `<project>/.gemini/lsp.*` |
...
- User-wide preferences → `~/.san/agent/lsp.json`
- Project-specific overrides → `<project>/.san/lsp.json`

> **Note:** Auto-detection is skipped only when at least one config file contributes server overrides. A config file that only sets `idleTimeoutMs` still lets San auto-detect built-in servers. When server overrides exist, San merges them with defaults and then loads servers that have matching `rootMarkers`, an available binary, and are not explicitly `disabled`.
...
Place the override in `<project>/.san/lsp.json`:
...
The user-level config in `~/.san/agent/lsp.json` is unaffected; pylsp is only suppressed in this project.
