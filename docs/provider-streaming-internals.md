...
This document explains how token/tool streaming is normalized in `@san/ai`, then propagated through `@san/agent` and `coding-agent` session events.
...
## Unified stream contract in `@san/ai`
