...
`san` resolves settings from built-in defaults, a persistent global config file, optional project-local config, one-shot CLI overlays, and in-memory runtime overrides. Reach for project settings when one repository needs a different provider set, model role, tool policy, memory backend, or UI behavior than your global defaults — without touching your machine-wide configuration.

Settings are stored as plain YAML mappings. Every key, its type, default, and enum values come from the settings schema, and you can inspect or change any of them with `san config` or the interactive `/settings` panel.
...
|---|---|---|---|
| Global | `~/.san/agent/config.yml` | The main persistent settings file. Always loaded. | `/settings`, `san config set`, and `san config reset` write here. |
| Global legacy JSON | `~/.san/agent/settings.json` (or legacy `~/.omp/agent/settings.json`) | Migrated into `config.yml` once, only when `config.yml` does not yet exist. | Not written after migration; the original is renamed to `settings.json.bak`. |
| Project | `<cwd>/.san/config.yml` (plus `.san/settings.json`) | Loaded when the process working directory has a non-empty `.san/`. | Read-only from settings commands; edit the file by hand. |
| Project legacy | `<cwd>/.san/settings.json` | Still read; project `config.yml` is merged on top of it. | Not written by settings commands. |
...
`SAN_CODING_AGENT_DIR` relocates the `~/.san/agent` base directory. When it is set, the global `config.yml`, the auth store (`agent.db`), and everything else under the agent directory move with it. Legacy `PI_CODING_AGENT_DIR` is still accepted. Use `san config path` to print the active agent directory.

Native project settings are intentionally scoped to the process working directory's `.san/` folder — settings discovery does **not** walk ancestor directories looking for the nearest `.san/`. Other discovery providers (Claude, Codex, Gemini, Cursor, OpenCode) can also contribute project-level settings from their own files; those are read-only from `san` settings commands and can be turned off by provider id (see [Provider and source disabling](#provider-and-source-disabling)).
...
Use the interactive `/settings` panel inside a session, or the `san config` command from a shell. Both operate on the merged effective settings, but every persistent write lands in the **global** file only.

```bash
san config list                 # all settings with current effective values
san config list --json          # same, machine-readable
san config get theme.dark       # one value
san config get theme.dark --json
san config set compaction.enabled false
san config set defaultThinkingLevel medium
san config reset steeringMode   # restore a key to its schema default
san config path                 # print the active agent directory
```

For users who want the full first-run animation on normal launches, set `startup.showSplash`:

```bash
san config set startup.showSplash true
```
...
| Command | Effect |
|---|---|
| `san config list` | Print every setting grouped by tab, with its current value and type. `--json` emits an object keyed by setting path with `{ value, type, description }`. |
| `san config get <key>` | Print the effective value of one key. Unknown keys exit non-zero. `--json` emits `{ key, value, type, description }`. |
| `san config set <key> <value>` | Parse `<value>` against the key's schema type and write it to the global `config.yml`. |
| `san config reset <key>` | Write the key's schema **default** back to the global config (this persists the default, it does not delete the key). |
| `san config path` | Print the active agent directory (honors `SAN_CODING_AGENT_DIR`; legacy `PI_CODING_AGENT_DIR` still works). |

`san config` with no subcommand, or `--help`, prints the help and lists settings. The `--json` flag is accepted by `list`, `get`, `set`, and `reset`.

### Value parsing

`san config set` parses the value string according to the target key's schema type. The string is trimmed first.
...
`san config set`, `san config reset`, `/settings`, and any runtime settings change all write to the global `config.yml` under the active agent directory. They never write to `<cwd>/.san/config.yml`. To create a project-local override, edit that file directly (see [Project-local config](#project-local-config)). Saves are debounced and re-read the file under a lock, so external edits made while a session is open are preserved.
...
3. **Project settings** — `<cwd>/.san/settings.json` then `<cwd>/.san/config.yml` (and contributions from other discovery providers at project level).
4. **Global settings** — `~/.san/agent/config.yml`.
...
Create `<repo>/.san/config.yml` when a repository needs its own settings:
...
```bash
san --config ./local/ci-settings.yml "check this failure"
san --config ./base.yml --config ./experiment.yml "try this model"
```
...
# <repo>/.san/config.yml — inside this repo ONLY groq is disabled
disabledProviders:
  - groq
```
...
Every key below is defined in the settings schema; `san config list` shows the full set with current values. Defaults and enum values are taken from the schema. Settings that accept an env or flag override are noted; those overrides are process-local and not persisted.
...
A value of `-1` means "use the provider/model default" — `san` does not send that parameter.
...
| `autolearn.enabled` | boolean | `false` | Experimental: after the agent stops, nudge it to capture lessons to memory and create/enhance isolated managed skills under `~/.san/agent/managed-skills`. Enables the `manage_skill` tool (and `learn` when a memory backend is active). |
...
`compaction` has additional tuning keys (idle compaction, supersede/drop heuristics) visible in `san config list`. See [Compaction](./compaction.md) for the full strategy reference.
...
| `auth.broker.url` | string | _(unset)_ | Auth-broker URL. Overridden by `SAN_AUTH_BROKER_URL` (legacy `OMP_AUTH_BROKER_URL` still works). |
| `auth.broker.token` | string | _(unset)_ | Auth-broker token. Overridden by `SAN_AUTH_BROKER_TOKEN` (legacy `OMP_AUTH_BROKER_TOKEN` still works). |
...
`san` migrates older config shapes automatically. None of these require action; they are listed so you know what changes you may see in `config.yml`.

### Startup migration to `config.yml`

When `~/.san/agent/config.yml` does not exist, startup builds it once from legacy sources, then writes the result:

1. `~/.san/agent/settings.json`, or legacy `~/.omp/agent/settings.json` when the active agent directory is under `.san` (renamed to `settings.json.bak` after a successful migration).
...
- Start `san` from the directory that contains `.san/config.yml`. Settings discovery only checks the current working directory's `.san/`, not ancestor directories.
- Ensure `.san/` is non-empty; empty config directories are ignored.
- Confirm the file is valid YAML and its top level is a mapping.
- Run `san config get <key>` from that directory to see the effective value.
...
### `san config set` changed the wrong file

`san config set` and `san config reset` always write the global `config.yml` under the active agent directory. Run `san config path` to print it. For project-local settings, edit `<repo>/.san/config.yml` directly.

### `san config reset` did not remove my key

`reset` writes the schema **default** value into the global config — it persists the default rather than deleting the key. To stop overriding a project value from global config, delete the key from `~/.san/agent/config.yml` by hand.
...
### `san config set <key>` says "Unknown setting"

Keys must match a schema path exactly, with no shorthand. Use `theme.dark`, not `theme`. Run `san config list` to see every valid key.
