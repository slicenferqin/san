...
The SDK is the in-process integration surface for `@san/coding-agent`.
...
```bash
bun add @san/coding-agent
```

## Entry points

`@san/coding-agent` exports the SDK APIs from the package root (and also via `@san/coding-agent/sdk`).
...
```ts
import { createAgentSession } from "@san/coding-agent";
...
```ts
import { createAgentSession, SessionManager } from "@san/coding-agent";
