...
`discoverAgents(cwd, home)` (`src/task/discovery.ts`) merges agents from San-native roots and Claude plugin roots before appending bundled definitions. Cross-harness roots such as `.claude/agents`, `.codex/agents`, and `.gemini/agents` are intentionally skipped because their frontmatter schema is not the San task-agent contract (`TASK_AGENT_CONFIG_SOURCE = ".san"` filters both dir lists).

### Discovery inputs

1. Nearest project `.san` agents dir from `findAllNearestProjectConfigDirs("agents", cwd)` (filtered to `.san`; first hit only)
2. User `.san` agents dir from `getConfigDirs("agents", { project: false })` (filtered to `.san`; first hit only)
...
1. project `.san/agents`
2. user `~/.san/agent/agents`
...
- Project `.san` overrides user `.san`.
...
`SAN_BLOCKED_AGENT` is read at tool construction; legacy `PI_BLOCKED_AGENT` still works. If request matches, execution is rejected with recursion-prevention message.
