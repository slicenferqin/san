...
`src/extensibility/hooks/index.ts` and the package subpath `@san/coding-agent/extensibility/hooks` export:
...
The package root (`@san/coding-agent`) does not re-export `HookAPI`; import legacy hook types from the hooks subpath.
