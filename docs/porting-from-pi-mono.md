...
- Examples (adjust to match the actual packages you are porting):
  - `@mariozechner/pi-coding-agent` → `@san/coding-agent`
  - `@mariozechner/pi-agent-core` → `@san/agent`
  - `@mariozechner/pi-tui` → `@san/tui`
  - `@mariozechner/pi-ai` → `@san/ai`
  - `@mariozechner/pi-utils` → `@san/utils`
- Some upstream packages publish under the `@earendil-works/*` scope instead of `@mariozechner/*`. Map it the same way (`@earendil-works/pi-coding-agent` → `@san/coding-agent`, and so on).
...
- In `packages/coding-agent`, use `logger` from `@san/utils` for internal/runtime logging; CLI command files may use `console.*` for intentional user-facing output.
...
- Prefer existing helpers and utilities over new ad-hoc code.
  ...
  - TypeScript packages generally use source files directly; `@san/natives` exports generated native bindings from `packages/natives/native`.
...
| Upstream                                           | Our Fork                                                  | Reason                                        |
| -------------------------------------------------- | --------------------------------------------------------- | --------------------------------------------- |
| `clipboard.ts` + `clipboard-image.ts` (tool files) | `src/utils/clipboard.ts` backed by `@san/natives` | Native implementation with a small TS wrapper |
...
| `formatSize` from `pi-coding-agent`                              | `formatBytes` from `@san/utils`                                                            |
...
- `clipboard-image.ts` — image clipboard support is exposed through `src/utils/clipboard.ts` backed by `@san/natives`
