...
┌───────────────────────────────────────┐
│ 1) ~/.san/agent, ~/.claude, ...       │
│ 2) <cwd>/.san, <cwd>/.claude, ...     │
└───────────────────────────────────────┘
                    │
                    ▼
        capability providers enumerate items
 (native provider scans project .san before user .san;
...
`CONFIG_DIR_NAME` is `.san` (`packages/utils/src/dirs.ts`).

## Profiles

A named profile (`san --profile <name>`, the `--alias` shortcut, or `SAN_PROFILE` / legacy `OMP_PROFILE` / `PI_PROFILE`) relocates the San user base. When a profile is active, every native user-level path written here as `~/.san/agent/...` resolves to `~/.san/profiles/<name>/agent/...` instead.
...
Walks parent directories upward and returns the **nearest existing directory per source base** (`.san`, `.claude`, `.codex`, `.gemini`), then sorts results by source priority.
...
1. Global settings: `~/.san/agent/config.yml`
2. Project settings: discovered via settings capability (`settings.json` and `config.yml` from providers)
3. CLI config overlays: `san --config <path>` / repeated `--config` files, loaded as `config.yml`-style YAML for this process only
...
1. Migrate from `~/.san/agent/settings.json` (or legacy `~/.omp/agent/settings.json`, renamed to `.bak` on success)
...
## 6) Native `.san` provider behavior (`packages/coding-agent/src/discovery/builtin.ts`)
...
- Skills scan `<ancestor>/.san/skills` for each ancestor from the current working directory up to the repo root/home boundary, plus `~/.san/agent/skills`, without requiring the root `.san` directory itself to be non-empty.
- `SYSTEM.md` and `AGENTS.md` read user-level files directly and use nearest-ancestor project `.san` lookup for project files, but the project `.san` directory must be non-empty. See [`docs/system-prompt-customization.md`](./system-prompt-customization.md) for the full `SYSTEM.md` / `APPEND_SYSTEM.md` contract (replace vs. append, templating).

### Scope-specific loading

- Skills: `<ancestor>/.san/skills/*/SKILL.md` and `~/.san/agent/skills/*/SKILL.md`
...
For `SYSTEM.md` and `AGENTS.md`, native provider uses nearest-ancestor project `.san` directory search (walk-up) and still requires the project `.san` dir to be non-empty.
