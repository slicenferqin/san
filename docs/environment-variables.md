...
Most runtime lookups use `$env` from `@san/utils` (`packages/utils/src/env.ts`).

`$env` loading order:

1. Existing process environment (`Bun.env`)
2. Project `.env` (`$PWD/.env`) for keys not already set
3. Agent `.env` (`~/.san/agent/.env`, respecting `SAN_CONFIG_DIR` / `SAN_CODING_AGENT_DIR`; legacy `PI_*` directory vars still work) for keys not already set
4. Config-root `.env` (`~/.san/.env`, respecting `SAN_CONFIG_DIR`; legacy `PI_CONFIG_DIR` still works) for keys not already set
5. Home `.env` (`~/.env`) for keys not already set

Additional rule inside each `.env` file: `SAN_*` keys are mirrored to `OMP_*` and `PI_*`; `OMP_*` keys are mirrored to `PI_*`.
...
| Variable                | Used for                                                                                     | Required when                                                                                                             | Notes / precedence                                                                                                                                                                         |
| ----------------------- | -------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `SAN_AUTH_BROKER_URL`   | Base URL of the remote auth-broker (e.g. `https://broker.tailnet:8765`); selects broker mode | Resolving credentials through a broker; also required by `san auth-gateway serve` (the gateway is itself a broker client) | Wins over legacy `OMP_AUTH_BROKER_URL` and `auth.broker.url` in `config.yml`. When set with no resolvable token, `resolveAuthBrokerConfig()` hard-errors instead of falling back to local SQLite.                           |
| `SAN_AUTH_BROKER_TOKEN` | Bearer token sent on every broker endpoint except `/v1/healthz`                              | `SAN_AUTH_BROKER_URL` is set and no token is available from `auth.broker.token` or `<config-dir>/auth-broker.token`       | Resolution: this env → legacy `OMP_AUTH_BROKER_TOKEN` → `auth.broker.token` (`$ENV_NAME` indirection supported) → `<config-dir>/auth-broker.token` (mode `0600`). `<config-dir>` is `~/.san/` by default. |
| `SAN_AUTH_BROKER_SNAPSHOT_TTL_MS` | Freshness window for the encrypted local broker snapshot cache | Optional in broker mode | Default `3600000` (1 h). Legacy `OMP_AUTH_BROKER_SNAPSHOT_TTL_MS` is still accepted. Freshness is based on broker `snapshot.generatedAt`; `0` disables cache reads/writes and forces the old blocking fetch every startup. |
| `SAN_AUTH_BROKER_SNAPSHOT_CACHE` | Path to the encrypted local broker snapshot cache | Optional in broker mode | Defaults to `~/.san/cache/auth-broker-snapshot.enc` (or XDG cache equivalent). Legacy `OMP_AUTH_BROKER_SNAPSHOT_CACHE` is still accepted. Useful for tests, ephemeral hosts, or relocating the `0600` cache file. |

The gateway has no dedicated env vars — it inherits `SAN_AUTH_BROKER_*` (legacy `OMP_AUTH_BROKER_*` still accepted). Its own inbound bearer token lives at `<config-dir>/auth-gateway.token` and is managed via `san auth-gateway token`.
...
SearXNG also reads the equivalent `searxng.endpoint`, `searxng.token`, `searxng.basicUsername`, and `searxng.basicPassword` settings from `~/.san/agent/config.yml`; environment variables are fallbacks.
...
| Variable                     | Default / behavior                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| ---------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `SAN_SMOL_MODEL`              | Ephemeral model-role override for `smol` (CLI `--smol` takes precedence; legacy `PI_SMOL_MODEL` still works)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| `SAN_SLOW_MODEL`              | Ephemeral model-role override for `slow` (CLI `--slow` takes precedence; legacy `PI_SLOW_MODEL` still works)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| `SAN_PLAN_MODEL`              | Ephemeral model-role override for `plan` (CLI `--plan` takes precedence; legacy `PI_PLAN_MODEL` still works)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| `PI_NO_TITLE`                | If set (any non-empty value), disables online auto session title generation on the first user message                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `SAN_TINY_DEVICE`            | ONNX execution provider for local memory/classification models; overrides `providers.tinyModelDevice` (default: CPU). Legacy `PI_TINY_DEVICE` also works at lower precedence. Supports `cpu`, `gpu`, `metal`/`webgpu`, `auto`, `cuda`, `dml`, `coreml`, `wasm`, `webnn`, `webnn-gpu`, `webnn-cpu`, `webnn-npu`                                                                                                                                                                                                                                              |
| `SAN_TINY_DTYPE`             | ONNX quantization/precision for local memory/classification models; overrides `providers.tinyModelDtype` (default: each model's shipped dtype, currently `q4`). Legacy `PI_TINY_DTYPE` also works at lower precedence. Supports `auto`, `fp32`, `fp16`, `q8`, `int8`, `uint8`, `q4`, `bnb4`, `q4f16`, `q2`, `q2f16`, `q1`, `q1f16`                                                                                                                                                                                                                        |
...
| `PI_SUBPROCESS_CMD`          | Overrides subagent spawn command (`san` / `san.cmd` resolution bypass)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| `SAN_PACKAGE_DIR`             | Overrides package asset base dir resolution (`docs/`, `examples/`, `CHANGELOG.md`); legacy `PI_PACKAGE_DIR` still works                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| `OLLAMA_CONTEXT_LENGTH`      | Positive integer context-window override for implicit Ollama discovery; affects San context budgeting only and does not change Ollama's runtime `num_ctx`                                                                                                                                                                                                                                                                                                                                                                                           |
...
| `SAN_NO_PTY`                  | If `1`, disables interactive PTY path for bash tool; legacy `PI_NO_PTY` still works                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `SAN_MCP_TIMEOUT_MS`          | Overrides MCP client request timeout (ms) for every MCP server. `0` disables client-side timeouts (`AbortSignal` never fires). Invalid (negative or non-numeric) values are ignored with a warning and the per-server config or default (`30000`) is used. Legacy `OMP_MCP_TIMEOUT_MS` still works.                                                                                                                                                                                                                                                                                         |

`SAN_NO_PTY` and legacy `PI_NO_PTY` are also set internally when CLI `--no-pty` is used.

---

## 6) Storage and config root paths

These are consumed via `@san/utils/dirs` and affect where coding-agent stores data.

| Variable              | Default / behavior                                                            |
| --------------------- | ----------------------------------------------------------------------------- |
| `SAN_CONFIG_DIR`       | Config root dirname under home (default `.san`; legacy `PI_CONFIG_DIR` still works)                               |
| `SAN_CODING_AGENT_DIR` | Full override for agent directory (default `~/<SAN_CONFIG_DIR or .san>/agent`; legacy `PI_CODING_AGENT_DIR` still works) |
