...
A **marketplace** is a Git repository (or local directory) containing a catalog file at `.san-plugin/marketplace.json` (preferred), `.omp-plugin/marketplace.json` (legacy San/OMP-compatible), or `.claude-plugin/marketplace.json` (Claude Code-compatible fallback). The catalog lists available plugins with their sources, descriptions, and metadata.

A **plugin** is a directory containing Claude/San plugin content such as skills, commands, agents, hooks, tools, MCP servers, or LSP servers. Extension modules (`package.json` `san.extensions` entry points; legacy `omp.extensions` remains accepted) are not loaded from marketplace installs — they only load for npm-installed or `san plugin link`ed plugins. Plugins are identified by `name@marketplace` (e.g. `code-review@claude-plugins-official`).

**Scopes**: marketplace plugins can be installed at two scopes:

- **user** (default) -- available in all projects, stored in `~/.san/plugins/installed_plugins.json`
- **project** -- available only in the active project, stored in the nearest project `.san/plugins/installed_plugins.json`
...
```
san plugin marketplace add <source>
san plugin marketplace remove <name>
san plugin marketplace update [name]
san plugin marketplace list
san plugin discover [marketplace]
san plugin install [--force] [--scope user|project] name@marketplace
san plugin uninstall [--scope user|project] name@marketplace
san plugin upgrade [--scope user|project] [name@marketplace]
san plugin enable [--scope user|project] name@marketplace
san plugin disable [--scope user|project] name@marketplace
```
...
Git and local sources must contain a catalog at `.san-plugin/marketplace.json` (preferred), `.omp-plugin/marketplace.json` (legacy San/OMP-compatible), or `.claude-plugin/marketplace.json` (Claude Code-compatible fallback). Direct catalog URLs cache only the JSON catalog; plugins in URL-sourced catalogs cannot use relative string sources like `"./plugins/foo"`.

## Catalog format (marketplace.json)

A marketplace catalog lives at `.san-plugin/marketplace.json` in the repository root. When San is the only intended consumer, prefer this path. For older San/OMP builds, publish `.omp-plugin/marketplace.json`; to remain Claude Code-compatible, publish `.claude-plugin/marketplace.json`. A repository may ship all three: San reads the `.san-plugin/` copy first, then `.omp-plugin/`, then `.claude-plugin/`. Same catalog format either way:
