...
- **`san auth-broker serve`** holds the canonical SQLite credential vault, performs OAuth refreshes, and exposes a small REST API (`/v1/snapshot`, `/v1/snapshot/stream`, `/v1/credential/:id/refresh`, `/v1/credential/:id/disable`, `/v1/credential`, `/v1/usage`, `/v1/healthz`).
- **`san auth-gateway serve`** is a forward-proxy. It accepts OpenAI Chat Completions, Anthropic Messages, OpenAI Responses, and San's native stream requests, resolves the broker-backed credential, and dispatches through the provider stack. Clients (containerised San, llm-git, the macOS usage widget, CI containers, IDE plugins, …) never see the access token.
...
```
                ...
                │                                                            │
  developer ──▶ │  ┌──────────────────────────┐    ┌────────────────────┐    │
  laptop /      │  │  san auth-broker serve   │◀──▶│  SQLite agent.db    │    │
  CI / San      │  │  - holds refresh tokens  │    │  (canonical writer)│    │
                ...
                │            ▼                                               │
                │  ┌──────────────────────────┐                              │
                │  │  san auth-gateway serve  │  RemoteAuthCredentialStore   │
...
```
san auth-broker serve     [--bind=host:port]                    # boot the broker
san auth-broker token     [--regenerate] [--json]               # print or rotate the bearer token
san auth-broker login     [<provider>] [--via=user@host] [--dry-run]
san auth-broker logout    [<provider>]
san auth-broker list      [--json]
san auth-broker import    <file|dir> [--provider=<id>] [--include-disabled] [--dry-run] [--json]
san auth-broker migrate   --from-local [--include-oauth] [--include-env] [--dry-run] [--json]
san auth-broker status    [--json]
```
...
- `login [<provider>]` runs the per-provider OAuth flow locally — when no provider is supplied, it falls back to an interactive numbered picker. With `--via=user@host` it shells out `ssh -L <callback-port>:127.0.0.1:<callback-port> user@host san auth-broker login <provider>` so the OAuth callback hits the local browser but the credential is written on the broker host (`--via` requires `<provider>`). Built-in callback ports: `anthropic:54545`, `openai-codex:1455`, `google-gemini-cli:8085`, `google-antigravity:51121`, `gitlab-duo:8080`. The OAuth dance is driven in-process via `AuthStorage.login()`.
...
- `import <file|dir>` imports CLIProxyAPI-style JSON credentials into the local SQLite store. Maps `type` field → San provider ids (`claude → anthropic`, `codex → openai-codex`, `gemini → google-gemini-cli`, `antigravity → google-antigravity`, `gemini-cli → google-gemini-cli`).
...
```
san auth-gateway serve   [--bind=host:port] [--no-auth]
san auth-gateway token   [--regenerate] [--json]
san auth-gateway status  [--json]
san auth-gateway check   [--strict] [--json]
```

- `serve` requires `SAN_AUTH_BROKER_URL` (or legacy `OMP_AUTH_BROKER_URL`, or `auth.broker.url` in `config.yml`) — the gateway is itself a broker client. It calls `AuthBrokerClient.fetchSnapshot()`, wraps it in `RemoteAuthCredentialStore`, and constructs an `AuthStorage` that resolves access tokens through the broker. Default bind is `127.0.0.1:4000`. The gateway token is stored at `<config-dir>/auth-gateway.token` (`0600`); `--no-auth` disables the bearer check entirely (loopback-only use).
...
The model id is read from the top-level `model` field for foreign wire formats and from the pi-native request body for `/v1/pi/stream`. The gateway picks the first bundled `Model<Api>` matching that id, parses the inbound wire format into a San request context, resolves the provider credential from broker-backed `AuthStorage`, dispatches through `streamSimple()`, and re-encodes the result to the inbound format (SSE for streamed responses).
...
`discoverAuthStorage()` persists the broker snapshot to `~/.san/cache/auth-broker-snapshot.enc` after the initial `/v1/snapshot` fetch and after later broker-sourced full snapshots. The file is AES-256-GCM encrypted with `SHA-256(SAN_AUTH_BROKER_TOKEN)` (legacy `OMP_AUTH_BROKER_TOKEN` also works) and authenticated with the broker URL as additional data, so changing either the token or URL makes the cache unreadable. The file is written atomically with mode `0600`.

Freshness is anchored to the broker-stamped `snapshot.generatedAt`, not local write time. Default TTL is 1 h (`SAN_AUTH_BROKER_SNAPSHOT_TTL_MS`; legacy `OMP_AUTH_BROKER_SNAPSHOT_TTL_MS` still works); `0` disables the cache and restores the old always-fetch boot path. When the cached snapshot is still fresh, San boots from it and skips the blocking `/v1/snapshot` query. `RemoteAuthCredentialStore` still starts its normal SSE / long-poll background sync immediately, so deleted or rotated credentials reconcile after startup, and expired OAuth access tokens still refresh through `POST /v1/credential/:id/refresh`.
...
The broker is **off** unless `SAN_AUTH_BROKER_URL` (or legacy `OMP_AUTH_BROKER_URL`, or `auth.broker.url` in `config.yml`) is set. When set, `discoverAuthStorage` in `packages/coding-agent/src/sdk.ts` swaps the local SQLite credential store for `RemoteAuthCredentialStore` and every API call resolves credentials through the broker.

### Environment variables

| Variable | Purpose | Required when |
| --- | --- | --- |
| `SAN_AUTH_BROKER_URL` | Base URL of the remote auth-broker (e.g. `https://broker.tailnet:8765`). Selecting this puts the client in broker mode — local SQLite is bypassed. Legacy `OMP_AUTH_BROKER_URL` still works. | Any time the San client should resolve credentials through a broker (and required by `san auth-gateway serve`). |
| `SAN_AUTH_BROKER_TOKEN` | Bearer token used for every broker endpoint except `/v1/healthz`. Legacy `OMP_AUTH_BROKER_TOKEN` still works. | When `SAN_AUTH_BROKER_URL` is set and no token is available from `auth.broker.token` or `<config-dir>/auth-broker.token`. |
| `SAN_AUTH_BROKER_SNAPSHOT_TTL_MS` | Freshness window for the encrypted local snapshot cache. Default `3600000` (1 h); `0` disables cache reads and writes. Legacy `OMP_AUTH_BROKER_SNAPSHOT_TTL_MS` still works. | Optional in broker mode. |
| `SAN_AUTH_BROKER_SNAPSHOT_CACHE` | Path override for the encrypted local snapshot cache. Default `~/.san/cache/auth-broker-snapshot.enc` (or XDG cache equivalent). Legacy `OMP_AUTH_BROKER_SNAPSHOT_CACHE` still works. | Optional in broker mode. |

Resolution order in `resolveAuthBrokerConfig()`:

1. `SAN_AUTH_BROKER_URL` env, then legacy `OMP_AUTH_BROKER_URL`, else `auth.broker.url` from `config.yml`, resolved through `resolveConfigValue`;
2. `SAN_AUTH_BROKER_TOKEN` env, then legacy `OMP_AUTH_BROKER_TOKEN`, else `auth.broker.token` from `config.yml`, else `<config-dir>/auth-broker.token`;
3. URL set but no token resolvable → hard error pointing at the token file path.

The gateway has no dedicated env vars — it inherits `SAN_AUTH_BROKER_*` because it is itself a broker client; legacy `OMP_AUTH_BROKER_*` aliases still work.

### `config.yml` keys

| Key                 | Default | Purpose                                                                                                                                                                            |
| ------------------- | ------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `auth.broker.url` | unset | Same as `SAN_AUTH_BROKER_URL`; env wins. Hidden from the settings UI. Values are resolved as a literal, an environment variable name, or `!<shell command>` to use trimmed stdout. |
| `auth.broker.token` | unset | Same as `SAN_AUTH_BROKER_TOKEN`; env wins. Values are resolved the same way. |

### Token files

| Path                              | Owner                                                | Mode                          |
| --------------------------------- | ---------------------------------------------------- | ----------------------------- |
| `<config-dir>/auth-broker.token` | `san auth-broker serve` (created at first start) | `0600` in a `0700` parent dir |
| `<config-dir>/auth-gateway.token` | `san auth-gateway serve` (skipped under `--no-auth`) | `0600` in a `0700` parent dir |

`<config-dir>` resolves to `~/.san/` (respecting `SAN_CONFIG_DIR`; legacy `PI_CONFIG_DIR` still works).
...
- [`secrets.md`](./secrets.md) — secret obfuscation around tokens that _do_ leak through (e.g. `SAN_AUTH_BROKER_TOKEN` in shell output).
- [`models.md`](./models.md) — provider auth resolution order; the broker plugs in at layers 2–3 (stored credentials).
- [`environment-variables.md`](./environment-variables.md) — full env reference including `SAN_AUTH_BROKER_URL` / `SAN_AUTH_BROKER_TOKEN` and legacy aliases.
