...
```text
~/.san/agent/sessions/<dir-encoded>/<timestamp>_<sessionId>.jsonl
```
...
```text
~/.san/agent/blobs/<sha256>
```

Terminal breadcrumb files are written under:

```text
~/.san/agent/terminal-sessions/<terminal-id>
```
...
- `listAllSessions(storage)` (a.k.a. `SessionManager.listAll()`) -> sessions across all project scopes under `~/.san/agent/sessions`
