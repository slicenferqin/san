 ...
 * pure protocol translation: foreign wire → San Context → pi-ai stream() →
 * San events → foreign wire.
 *
 * Endpoints:
...
import { extractHttpStatusFromError, extractRetryHint, logger } from "@san/utils";
...
function deriveSessionId(modelId: string, context: Context): string {
	...
	if (first) {
		...
		// of the same conversation (San re-stamps every parsed Message). role +
