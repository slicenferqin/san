 ...
 * (containerized San, the swarm extension, robomp's sidecar auth-gateway).
 ...
 * `SimpleStreamOptions` from older / newer San builds without per-version
 * conditionals.
 */
 ...
 * targets (containerized San → host gateway, robomp slot → legacy
 * `omp-auth-gateway` sidecar) the bandwidth cost is negligible compared to provider latency —
