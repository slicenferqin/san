...
import { withAuth } from "@san/ai";
import { type AuthCredentialStore, AuthStorage, SqliteAuthCredentialStore } from "@san/ai/auth-storage";
import { ProviderHttpError } from "@san/ai/error";
import { registerOAuthProvider, unregisterOAuthProviders } from "@san/ai/registry/oauth";
import type { CredentialRankingStrategy, UsageProvider } from "@san/ai/usage";
...
describe("AuthStorage forceRefresh + rotateSessionCredential", () => {
	...
	test("rotateSessionCredential(usage-limit) delegates to markUsageLimitReached", async () => {
		...
		expect(rotated).toBe(true);
		// 账号额度错误交给 markUsageLimitReached，并透传服务端声明的恢复窗口。
		...
		expect(usageLimitSpy.mock.calls[0]?.[2]?.retryAfterMs).toBe(158 * 60_000);
