...
import type { ApiKeyResolveContext } from "@san/ai";
import { registerCustomApi, unregisterCustomApis } from "@san/ai";
import { ProviderHttpError } from "@san/ai/error";
import { classify } from "@san/ai/error/flags";
import { streamSimple } from "@san/ai/stream";
import type { Api, AssistantMessage, Context, Model, SimpleStreamOptions, Usage } from "@san/ai/types";
...
describe("streamSimple resolver auth retry", () => {
	...
	it("rotates directly to a sibling when a 403 is thrown before the first event", async () => {
		const keys: unknown[] = [];
		const contexts: ApiKeyResolveContext[] = [];
		registerCustomApi(
			API,
			(_model: Model<Api>, _context: Context, options?: SimpleStreamOptions) => {
				pushKey(keys, options);
				const stream = new AssistantMessageEventStream();
				queueMicrotask(() => (keys.length === 1 ? stream.fail(authError(403)) : ok(stream)));
				return stream;
			},
			SOURCE_ID,
		);
	...
	it("surfaces a 403 concurrency cap without rotating credentials", async () => {
		const keys: unknown[] = [];
		const contexts: ApiKeyResolveContext[] = [];
		...
		const stream = streamSimple(model(), context, {
			apiKey: async ctx => {
				contexts.push(ctx);
				return ctx.error === undefined ? "old-key" : "sibling-key";
			},
		});
		await expect(
			(async () => {
