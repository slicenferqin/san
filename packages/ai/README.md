...
```typescript
import { z, getModel, stream, complete, Context, Tool } from "@san/ai";
...
```typescript
import { stream, validateToolCall, Tool } from "@san/ai";
...
```typescript
import * as fs from "node:fs";
import { getModel, complete } from "@san/ai";
...
```typescript
import { getModel, streamSimple, completeSimple } from "@san/ai";
...
```typescript
import { getModel, stream } from "@san/ai";
...
```typescript
import { getProviders, getModels, getModel } from "@san/ai";
...
```typescript
import { Model, stream } from "@san/ai";
...
```typescript
import { getModel, complete, Context } from "@san/ai";
...
```typescript
import { Context, getModel, complete } from "@san/ai";
...
bundles. Bun's `fetch` does not consume the env var natively, so San injects
...
```typescript
import { getEnvApiKey } from "@san/ai";
...
Authenticate via the [`san`](https://github.com/slicenferqin/san) coding-agent CLI, which drives this library's OAuth/API-key flows in-process and persists into `agent.db`:

```bash
san auth-broker login              # interactive provider selection
san auth-broker login anthropic    # login to a specific provider
san auth-broker login vllm         # store vLLM API key (or placeholder for local no-auth)
san auth-broker list               # list supported providers
san auth-broker logout             # interactive — pick a stored credential to remove
```
...
```typescript
import { loginGitHubCopilot } from "@san/ai";
import * as fs from "node:fs";
...
```typescript
import { getModel, complete, getOAuthApiKey } from "@san/ai";
import * as fs from "node:fs";
