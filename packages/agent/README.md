...
Stateful agent with tool execution and event streaming. Built on `@san/ai`.

## Installation

```bash
npm install @san/agent
```
...
Define tools using `AgentTool` with a Zod parameter schema (via `z` from `@san/ai`).
...
```typescript
import { Agent, streamProxy } from "@san/agent";
...
```typescript
import { agentLoop, agentLoopContinue } from "@san/agent";
