...
import { workerHostEntry } from "@san/utils";
import {
...
function defaultWorkerCount(): number {
	...
	// the compiled `san` binary. Keep macOS on the documented serial path.
 ...
 * self-dispatching CLI entry (San in source, npm-bundle, or compiled form),
 * re-enter that entry with a worker argv selector; otherwise (standalone
 * san-stats, bun test, SDK embedding) load the worker module directly, so this
 * package keeps zero runtime dependency on `@san/coding-agent`.
 */
 ...
 * then terminates. Used by `san --smoke-test` so the install-method CI jobs
