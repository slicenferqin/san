...
Multi-agent orchestration for San. Define agent workflows in YAML — pipelines, parallel fan-outs, sequential chains, or any DAG — and run them unattended until completion.

Each agent is a full San subagent with access to every tool: bash, python, read, write, edit, grep, find, fetch, web_search, browser. The orchestrator manages lifecycle and ordering; agents communicate through the shared workspace filesystem.
...
san-swarm path/to/swarm.yaml

# Background — survives terminal close:
nohup san-swarm path/to/swarm.yaml \
...
Register the extension in your config (`~/.san/agent/settings.json` or `.san/settings.json`):
...
Each agent is a full San session. It can:
...
Any model configured in San works. Set a swarm default and optionally override per agent:
...
src/swarm/
  ...
  executor.ts         Spawns agents via San's runSubprocess
