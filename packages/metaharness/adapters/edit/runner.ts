...
import { formatHashlineHeader, InMemorySnapshotStore } from "@san/hashline";
import type { AgentMessage, ResolvedThinkingLevel, ThinkingLevel } from "@san/agent";
import type { Model, ToolExample } from "@san/ai";
import { formatSessionDumpText, RpcClient } from "@san/coding-agent";
import { prompt } from "@san/utils";
import { diffLines } from "diff";
import { formatDirectory } from "@san/typescript-edit-benchmark/formatter";
import {
	discoverSharedInfra,
	InProcessClient,
	type SharedInfra,
} from "@san/typescript-edit-benchmark/in-process-client";
...
import type { EditTask } from "@san/typescript-edit-benchmark/tasks";
import {
	verifyExpectedFileSubset,
	verifyExpectedFiles,
} from "@san/typescript-edit-benchmark/verify";

const REPO_ROOT = path.resolve(import.meta.dir, "..", "..", "..", "..");
const RUNS_DIR = path.join(REPO_ROOT, "runs");
const TMP = path.join(RUNS_DIR, `rb-${Math.random().toString(36).slice(2, 10)}`);
const CLI_PATH = Bun.fileURLToPath(import.meta.resolve("@san/coding-agent/cli"));
