...
1. **Local San, not npm.** By default the runner bind-mounts the repo
   read-only into each task container (`--install source`) and runs San
   ...
   `/opt/san/bin` — so trial setup needs zero outbound network. Alternatives:
   `--install local` (pack a tarball per run) or `--binary` (prebuilt
   `dist/san-linux-*` self-contained binaries).
...
| `--agent-arg <arg>` | — | Extra arg forwarded verbatim to the in-container San CLI (repeatable) |
| `--env <KEY[=VALUE]>` | — | Forward env into the San container (repeatable); `KEY` alone forwards the host value |
| `--binary <path>` | — | Prebuilt `san` binary (repeat for arm64+x64) |
| `--install <source\|local\|published>` | `source` | `source` = repo bind-mount, `local` = tarball pack, `published` = npm `@san/coding-agent` |
...
arc; ~$0.001 per report). API keys resolve through San's auth storage.
