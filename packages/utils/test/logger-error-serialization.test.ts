...
beforeAll(() => {
	tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "san-logger-error-"));
	// `getLogsDir()` honors SAN_CODING_AGENT_DIR / HOME for its base; pin to our tmp.
	...
	prevAgentDir = process.env.SAN_CODING_AGENT_DIR;
	process.env.SAN_CODING_AGENT_DIR = tempDir;
...
afterAll(() => {
	if (prevAgentDir === undefined) {
		...
		process.env.SAN_CODING_AGENT_DIR = prevAgentDir;
	}
...
async function waitForLogEntry(targetMessage: string): Promise<Record<string, unknown>> {
	for (let i = 0; i < 40; i++) {
		const files = fs.readdirSync(tempDir).filter(f => f.startsWith("san.") && f.endsWith(".log"));
