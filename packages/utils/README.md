...
Shared utilities for [San](https://github.com/slicenferqin/san) packages. Zero ceremony, Bun-first.

## Notable modules

| Module | Purpose |
| --- | --- |
| `logger` | Centralized logger writing to `~/.san/logs/` with rotation (TUI-safe — never stdout) |
| `prompt` | Handlebars-based prompt templating and formatting helpers |
| `dirs` | Path helpers for San config directories (`~/.san`, XDG-aware on Linux) |
...
Import from the root barrel or per-module subpaths (`@san/utils/<module>`).
...
- [Monorepo README](https://github.com/slicenferqin/san#readme)
