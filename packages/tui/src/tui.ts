...
import { $flag, getDebugLogPath, logger } from "@san/utils";
...
/** Controls how a settled resize refreshes transcript rows retained by the terminal. */
...
export interface HistoryBatch {
	...
	/**
	 * `append` adds finalized rows to native history. `replay` replaces the
	 * terminal's erased history with the complete logical ledger in one frame.
	 */
 ...
 * Callers MUST NOT mutate it. Components MAY return a cached array while their
 * rendered content is unchanged and MUST return a fresh array when it changes.
 * The frame provider owns immutable history and mutable viewport boundaries.
 */
...
export class TUI extends Container {
	...
	// Snapshot ledger used only by hosts that do not install an explicit frame
	// provider. It preserves the public TUI scrollback behavior without making
	// the provider contract infer history from rendered rows.
	#fallbackRows: string[] = [];
	#fallbackWidth = 0;
	#fallbackBoundary = 0;
	...
	static #initialResizeScrollbackMode(): ResizeScrollbackMode {
		const mode = Bun.env.SAN_TUI_RESIZE_SCROLLBACK;
	...
	setFrameProvider(provider: TerminalFrameProvider | undefined): void {
		const hadRenderedFrame = this.#hasEverRendered;
		...
		this.#fallbackBoundary = 0;
		this.requestRender(true, { clearScrollback: hadRenderedFrame });
	}

	/** Current policy for refreshing settled transcript rows after a resize. */
	getResizeScrollback(): ResizeScrollbackMode {
		return this.#resizeScrollbackMode;
	}

	/** Set how settled resizes refresh transcript rows retained by the terminal. */
	setResizeScrollback(mode: ResizeScrollbackMode): void {
		if (this.#resizeScrollbackMode === mode) return;
	...
	#resizeRepaintsInPlace(): boolean {
		...
		return isInsideTerminalMultiplexer() || Bun.env.TERM_PROGRAM?.toLowerCase() === "warpterminal";
	}
	...
	#beginResizeAltPaint(): void {
		...
		this.#resizeSettleTimer = this.#renderScheduler.scheduleRender(() => {
			...
			if (this.#frameProvider === undefined) {
				// Hosts without a frame provider have no logical transcript anchor to
				// probe. Repaint directly after the borrowed resize buffer is restored;
				// the fallback ledger will rebuild at the new width.
				this.#forceViewportRepaintOnNextRender = true;
				this.requestRender(true);
			} else {
	...
	#renderResizeAltFrame(width: number, height: number): void {
		...
		let viewport = rendered.length > height ? rendered.slice(rendered.length - height) : Array.from(rendered);
		if (provider === undefined && this.#getTopmostVisibleOverlay() !== undefined) {
	...
	#flushHistoryBeforeStop(): void {
		...
		try {
			while (true) {
				...
				const viewport = plan.viewport.length > height ? plan.viewport.slice(0, height) : Array.from(plan.viewport);
				...
				if (this.#acceptedHistoryBatchId <= acceptedBefore) {
					throw new Error("History flush did not accept a new batch");
				}
			}
		} finally {
	...
	stop(): void {
		...
		// A pending destructive repaint belongs to the next live frame; shutdown
		// must flush only rows not already accepted by the terminal.
	...
	renderNow(options?: RenderRequestOptions): void {
		...
		this.#doRender();
		this.#lastFrameCostMs = Math.ceil(Math.max(0, this.#renderScheduler.now() - start) * 1_000_000) / 1_000_000;
	}
	...
	/** DECCARA is safe only inside synchronized output on terminals that advertise it. */
	...
	#emitPlanFrame(
		...
		const coordinateEpochChanged = !geometryStable || historyRows.length > 0 || startTop !== newTop;
		if (!destructiveReset) {
			// A resize/reflow invalidates physical row coordinates. Treat the old
			// attachment as archived conservatively before starting the new epoch.
			this.#imageBudget.observeCommitWatermark(geometryStable ? pushed : Number.MAX_SAFE_INTEGER);
			if (coordinateEpochChanged) this.#imageBudget.beginPlacementCoordinateEpoch();
		}
		if ($flag("PI_DEBUG_REDRAW")) {
		...
		if (destructiveReset && !isInsideTerminalMultiplexer()) buffer += "\x1b[H\x1b[3J\x1b[2J";
		const diffable =
			geometryStable &&
		...
		if (diffable) {
			let index = 0;
			while (index < rows) {
				if (this.#providerWindow[index] === prepared[index]) {
					index++;
					continue;
				}
				const firstChanged = index;
				while (index < rows && this.#providerWindow[index] !== prepared[index]) index++;
				const changed = prepared.slice(firstChanged, index);
				let changedTexts = changed;
				let fillSequence = "";
				if (this.#deccaraFillsEnabled()) {
					const plan = planDeccaraFills(changed, width, newTop + firstChanged);
					changedTexts = plan.texts;
					fillSequence = plan.sequence;
				}
				for (let offset = 0; offset < changedTexts.length; offset++) {
					const row = firstChanged + offset;
					buffer += `\x1b[${newTop + row + 1};1H${this.#lineRewriteSequence(
						changedTexts[offset] ?? "",
						width,
						newTop + row,
						newTop + row,
						-1,
						this.#osc66SpacerGlyphWidth(prepared, row),
					)}`;
				}
				buffer += fillSequence;
			}
			...
			let viewportTexts = prepared;
			let fillSequence = "";
			if (this.#deccaraFillsEnabled()) {
				const plan = planDeccaraFills(prepared, width, newTop);
				viewportTexts = plan.texts;
				fillSequence = plan.sequence;
			}
		...
		if (target) {
			...
			// Park the hidden cursor on the mutable viewport's top row so resize
			// anchor recovery follows the live tail rather than replayed history.
		...
		if (history !== undefined) {
			...
			// Explicit providers may need a follow-up frame after acknowledgement
			// to retire the offered prefix from their mutable viewport. The fallback
			// already supplied its complete composed viewport, so scheduling another
			// frame would create an unnecessary repaint and can delay input handling.
			if (provider !== undefined && history.kind !== "replay" && !this.#flushingHistory) {
				this.requestRender();
			}
		}
	}
	...
	/**
	 * Fallback frame for hosts without a frame provider (tests, simple embeds).
	 * The fallback keeps the historical public TUI behavior: finalized rows are
	 * written to native scrollback once, while the bottom window remains mutable.
	 * Provider hosts use the explicit batch contract instead.
	 */
	#renderChildrenFrame(width: number, height: number): void {
		this.#imageBudget.beginPass();
		const composed = Array.from(this.render(width));
		this.#imageBudget.endPass();
		if (this.#maybeDeferGhosttyInitialImagePaint()) return;

		const boundary = Math.max(0, composed.length - height);
		const previous = this.#fallbackRows;
		const widthChanged = this.#fallbackWidth > 0 && this.#fallbackWidth !== width;
		const forcedReset = this.#clearScrollbackOnNextRender;
		let replay = !this.#hasEverRendered || widthChanged || forcedReset;
		if (!replay && previous.length > 0) {
			const commonPrefix = Math.min(this.#fallbackBoundary, boundary, previous.length, composed.length);
			for (let index = 0; index < commonPrefix; index++) {
				if (previous[index] !== composed[index]) {
					replay = true;
					break;
				}
			}
			if (!replay && boundary < this.#fallbackBoundary) replay = true;
			if (!replay && composed.length < previous.length && boundary <= this.#fallbackBoundary) replay = true;
		}

		let history: HistoryBatch | undefined;
		if (replay) {
			this.#clearScrollbackOnNextRender = true;
			history = { id: this.#acceptedHistoryBatchId + 1, kind: "append", rows: composed.slice(0, boundary) };
		} else if (boundary > this.#fallbackBoundary) {
			history = {
				id: this.#acceptedHistoryBatchId + 1,
				kind: "append",
				rows: composed.slice(this.#fallbackBoundary, boundary),
			};
		}

		const viewport = composed.length > height ? composed.slice(composed.length - height) : composed;
		this.#emitPlanFrame(width, height, viewport, history, undefined);
		this.#fallbackRows = composed;
		this.#fallbackWidth = width;
		this.#fallbackBoundary = boundary;
	}
	...
	#emitAltFrame(lines: string[], width: number, height: number): void {
		...
		let paintLines = fitted;
		let fillSequence = "";
		if (this.#deccaraFillsEnabled()) {
			const plan = planDeccaraFills(fitted, width);
			paintLines = plan.texts;
			fillSequence = plan.sequence;
		}
		...
		for (let r = 0; r < height; r++) {
			...
			buffer += this.#lineRewriteSequence(
				...
				-1,
				-1,
				this.#osc66SpacerGlyphWidth(fitted, r),
			);
		}
		buffer += fillSequence;
		buffer += this.#paintEndSequence;
