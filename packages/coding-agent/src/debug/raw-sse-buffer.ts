import type { Model, ProviderResponseMetadata, RawSseEvent } from "@san/ai";
...
function trimRawLines(raw: string[]): TrimResult {
	...
	const tail = `: san-debug-truncated originalChars=${originalChars}`;
	trimmed.push(tail);
	chars += tail.length + 1;
...
export class RawSseDebugBuffer {
	...
	toRawText(): string {
		...
		const dropped = `: san-debug-dropped records=${this.#droppedRecords} chars=${this.#droppedChars}\n\n`;
