...
import type { WorkProfile } from "@san/natives";
import { APP_NAME, getLogPath, getLogsDir, getReportsDir, isEnoent } from "@san/utils";
...
export async function createReportBundle(options: ReportBundleOptions): Promise<ReportBundleResult> {
	...
	const outputPath = path.join(reportsDir, `san-report-${timestamp}.tar.gz`);
 ...
 * after a crash still captures the fatal PID's `san.<date>.<pid>.log`. Files
