...
echo "# Shell snapshot - generated by San agent" >| "$SNAPSHOT_FILE"
...
${fnEnvHelper}
printf '%s\\n' "$__san_funcs" | __san_emit_referenced_exports >> "$SNAPSHOT_FILE"
unset -f __san_sq_quote __san_emit_export_for __san_emit_referenced_exports 2>/dev/null
unset __san_funcs __san_qbuf __san_qout __san_sq __san_xv __san_name 2>/dev/null
...
export async function getOrCreateSnapshot(
	...
	const snapshotDir = path.join(os.tmpdir(), "san-shell-snapshots");
