...
import { formatBytes, readImageMetadata, SUPPORTED_IMAGE_MIME_TYPES } from "@san/utils";
 ...
 * to PNG/JPEG — the automatic equivalent of `SAN_NO_WEBP=1` (legacy: `OMP_NO_WEBP`).
 */
 ...
 * canonical `SAN_NO_WEBP` or legacy `OMP_NO_WEBP` env fallback in {@link resizeImage} still applies. Feed straight
...
export interface LoadImageInputOptions {
	...
	/** Force non-WebP output (e.g. for Ollama). Leave unset to honor SAN_NO_WEBP and its legacy alias. */
	excludeWebP?: boolean;
}
