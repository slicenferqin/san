...
/**
 * Read canonical `SAN_NO_WEBP` and legacy `OMP_NO_WEBP` per call so runtime toggles take effect.
...
function isWebPExcluded(): boolean {
	const raw = Bun.env.SAN_NO_WEBP ?? Bun.env.OMP_NO_WEBP;
 ...
 *
 * Set SAN_NO_WEBP to exclude WebP from encoding (llama.cpp STB doesn't decode it).
 *
...
export async function resizeImage(img: ImageContent, options?: ImageResizeOptions): Promise<ResizedImage> {
	...
	try {
		...
		// WebP usually beats JPEG by 25–35% but is disabled when SAN_NO_WEBP (or its legacy alias) is set
		...
		// unless SAN_NO_WEBP (or its legacy alias) is set (llama.cpp STB incompatibility).
