...
#
# `__san_emit_referenced_exports` reads captured function bodies from stdin,
...
__san_sq_quote() {
	...
	__san_qout=
	__san_sq=\'
	while case "$__san_qbuf" in *$__san_sq*) true ;; *) false ;; esac; do
		__san_qout=$__san_qout${__san_qbuf%%$__san_sq*}"'\\''"
		__san_qbuf=${__san_qbuf#*$__san_sq}
	done
	__san_qout=$__san_qout$__san_qbuf
	printf "'%s'" "$__san_qout"
}
...
__san_emit_export_for() {
	...
	eval "__san_xv=\"\${$1}\"" 2>/dev/null || return
	printf 'export %s=%s\n' "$1" "$(__san_sq_quote "$__san_xv")"
}

# Read function bodies on stdin, emit dedup'd export lines on stdout.
__san_emit_referenced_exports() {
	grep -oE '\$\{?[A-Za-z_][A-Za-z0-9_]*' \
		| sed -E 's/^\$\{?//' \
		| sort -u \
		| while IFS= read -r __san_name; do
			__san_emit_export_for "$__san_name"
		done
}
