...
export interface ModelHubCallbacks {
	...
	onPick?: (model: Model, selector: string, thinkingLevel: ConfiguredThinkingLevel | undefined) => void;
...
export interface ModelHubOptions {
	...
	/** Hub behavior: role/fallback management or a session-only model pick. */
	mode?: ModelHubMode;
...
const PICK_MODE_HINT = "Session-only switch — role assignments stay unchanged";
...
export class ModelHubComponent implements Component {
	...
	#activateItem(item: ModelBrowserItem): void {
		if (this.#mode === "pick") {
			if (item.model.reasoning && getSupportedEfforts(item.model).length > 0) {
				this.#openThinkingStrip(item, undefined, false);
				return;
			}
			this.#callbacks.onPick?.(item.model, item.selector, undefined);
			return;
		}
		if (this.#assigning) {
	...
	#openThinkingStrip(item: ModelBrowserItem, role: string | undefined, returnToRoles: boolean): void {
		const options =
			role === undefined
				? [ThinkingLevel.Off, AUTO_THINKING, ...getSupportedEfforts(item.model)]
				: this.#thinkingOptionsFor(item.model);
		const current =
			role === undefined
				? (this.#initialThinkingLevel ?? AUTO_THINKING)
	...
	#activateStripChip(): void {
		...
		switch (chip.action) {
			...
			case "thinking":
				...
				if (this.#mode === "pick" && strip.role === undefined) {
					this.#closeStrip();
					this.#callbacks.onPick?.(strip.item.model, strip.item.selector, chip.thinkingLevel);
					return;
				}
				if (strip.role) {
	...
	#statusRow(width: number): string {
		...
		if (this.#mode === "pick") {
			return truncateToWidth(theme.fg("muted", ` ${this.#pickerHint}`), width);
		}
		const entry = this.#activeEntry();
	...
	#footerHint(): string {
		...
		if (this.#mode === "pick") {
			return "Enter use for this session · ↑/↓ models · type to search · Esc close";
		}
		const entry = this.#activeEntry();
		if (entry.kind === "roles") {
			if (this.#focus !== "list") {
	...
	#renderFooter(width: number): string {
		...
		const prefix =
			strip.kind === "role"
				...
				: strip.role === undefined
					? `${theme.fg("accent", "Effort for session")}${theme.fg("dim", ` · ${strip.item.id} →`)} `
