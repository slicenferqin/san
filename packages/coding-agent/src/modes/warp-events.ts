...
import { isInsideTmux, wrapTmuxPassthrough } from "@san/tui/terminal-capabilities";
	...
	return {
		emit(event): void {
			...
			const body = {
				...
				// Warp resolves this via CLIAgent.command_prefix(); keep the registered
				// OhMyPi compatibility identifier until Warp recognizes San directly.
				agent: "omp",
	...
	return api => {
		...
		api.on("tool_approval_requested", event => {
			emitter?.emit({
				...
				summary: `San wants to run ${event.toolName}`,
			});
		});
