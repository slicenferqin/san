...
import { type AutocompleteProvider, matchesKey, type SlashCommand } from "@san/tui";
import { $env, isEnoent, logger, sanitizeText } from "@san/utils";
...
export class InputController {
	...
	setupKeyHandlers(): void {
		...
		// Alt+P opens the compact session picker; Alt+M opens the full session picker.
		// Role assignment remains available through `/model roles`.
	...
	handleCtrlZ(): void {
		...
		try {
			...
			// longer stops san: tokio swallows it and the TUI ends up torn down
	...
	async openExternalEditor(): Promise<void> {
		...
		try {
			...
			const result = await openInEditor(editorCmd, currentText, { extension: ".san.md", stdio });
			if (result !== null) {
