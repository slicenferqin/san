...
const LARGE_LOGO = SAN_LOGO.flatMap(line => {
	let wide = "";
...
/**
 * Animated setup splash: the brand π
...
function renderCompactSplash(width: number, height: number, phase: number, shine: ShineConfig): string[] {
	const art = height >= 14 ? LARGE_LOGO : SAN_LOGO;
	const content = [...gradientLogo(art, phase, shine), "", theme.bold("S a n")];
