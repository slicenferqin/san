...
import { APP_NAME, getActiveProfile, MIN_BUN_VERSION, resolveProfileEnv, setProfile, VERSION } from "@san/utils/dirs";
import { declareWorkerHostEntry, installWorkerInbox } from "@san/utils/worker-host";
...
// `@san/utils/env` eagerly loads `.env` from the agent directory at
// import time, so it must not be imported before `setProfile` runs.

async function showHelp(config: CliConfig): Promise<void> {
	const { renderRootHelp } = await import("@san/utils/cli");
...
async function runWorkerEntrypoint(arg: string | undefined): Promise<boolean> {
	...
	if (arg === STATS_SYNC_WORKER_ARG) {
		...
		await import("@san/stats/sync-worker");
...
export async function runCli(argv: string[]): Promise<void> {
	...
	try {
		...
		if (extracted.profile !== undefined) {
			...
			// No explicit --profile: activate any SAN_PROFILE/OMP_PROFILE/PI_PROFILE inherited
			...
			// SAN_PROFILE after importing this module (profile aliases/tests). Surfacing
			// validation here turns `SAN_PROFILE=.. san --version` into a clean error;
			// calling setProfile keeps every later path helper on the env-selected
			// profile instead of the default agent directory.
			setProfile(resolveProfileEnv(process.env.SAN_PROFILE, process.env.OMP_PROFILE, process.env.PI_PROFILE));
		}
		if (extracted.aliasName !== undefined) {
			const profile = extracted.profile ?? getActiveProfile();
			if (!profile) {
				throw new Error("--alias requires --profile <name> or SAN_PROFILE");
			}
	...
	// `@san/utils/env` here would snapshot the wrong agent `.env`.
