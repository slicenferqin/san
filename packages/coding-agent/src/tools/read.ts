...
import { formatHashlineHeader, formatNumberedLine, formatNumberedLines } from "@san/hashline";
import { glob, type SummaryResult, summarizeCode } from "@san/natives";
...
export class ReadTool implements AgentTool<typeof readSchema, ReadToolDetails> {
	...
	#pdfImageCacheDir(absolutePdfPath: string): string {
		...
		if (root === undefined) {
			...
			root = sessionFile?.endsWith(".jsonl")
				...
				: path.join(os.tmpdir(), "san-read-pdf-images");
		}
	...
	async execute(
		...
		// Handle internal URLs (agent://, artifact://, history://, memory://, skill://, rule://, local://, mcp://, san://, legacy omp://, issue://, pr://, vault://).
