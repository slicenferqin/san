...
import { ImageProtocol, TERMINAL } from "@san/tui";
import { getProjectDir, isEnoent, logger, prompt } from "@san/utils";
 ...
 * ACP deployment shape of an editor spawning San as a co-hosted subprocess.
 */
...
export class BashTool implements AgentTool<typeof bashSchemaBase | typeof bashSchemaWithAsync, BashToolDetails> {
	...
	async execute(
		...
		// 仅提取一个裸路径 token；其余 shell 语法保留给 shell 自身解释。
		if (!cwd) {
			const leadingCd = extractLeadingCdTarget(command);
			if (leadingCd) {
				cwd = leadingCd.path;
				command = leadingCd.rest;
			}
		}
		...
		// must still cancel the call or job, but San does not impose a deadline.
