 ...
 * updated immediately. Internal artifacts (for example files below
 * `~/.san/agent/sessions/`) and other scheme URLs are always written directly
 * to disk — those are San-owned and should never be pushed into the editor.
 */
 ...
 *
 * Returns `false` for internal-URL paths (for example a plan below
 * `~/.san/agent/sessions/`) and for the active plan file while plan mode is
 * enabled — both are San-internal artifacts that must stay off the editor buffer.
 */
export function shouldRouteWriteThroughBridge(
	...
	// San-owned session artifacts (plan files, scratch notes) must stay off the
