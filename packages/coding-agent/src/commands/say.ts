...
export default class Say extends Command {
	...
	static examples = [
		'san say "hello world"',
		"san say --file notes.md --voice bm_fable",
		'san say "hello world" --out /tmp/hello.wav',
		'san say "bonjour" --voice af_heart --model kokoro',
	];
	...
	#synthesisFailed(model: string): void {
		process.stderr.write(
			chalk.red(
				`error: could not synthesize with local TTS model "${model}". Run \`san setup speech\` to install it.\n`,
			),
		);
	}
}
