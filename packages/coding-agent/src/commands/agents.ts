...
export default class Agents extends Command {
	...
	static flags = {
		...
		user: Flags.boolean({ description: "Write to ~/.san/agent/agents (default)" }),
		project: Flags.boolean({ description: "Write to ./.san/agents" }),
	};

	static examples = [
		"# Export bundled agents into user config (default)\n  san agents unpack",
		"# Export bundled agents into project config\n  san agents unpack --project",
		"# Overwrite existing local agent files\n  san agents unpack --project --force",
		"# Export into a custom directory\n  san agents unpack --dir ./tmp/agents --json",
	];

	async run(): Promise<void> {
		const { args, flags } = await this.parse(Agents);
		if (!args.action) {
			renderCommandHelp("san", "agents", Agents);
			return;
		}
