 ...
 *
 * CLI-provided paths are injected via {@link injectSanExtensionCliRoots}
 * before discovery runs; settings paths are read lazily from
 * `<scope>/settings.json` in {@link listSanExtensionRoots} to mirror what
...
import { CONFIG_DIR_NAME, getAgentDir, isEnoent, logger, tryParseJson } from "@san/utils";
...
export interface SanExtensionRoot {
 ...
 * the registered set; {@link clearSanExtensionCliRoots} resets for tests.
 */
export function injectSanExtensionCliRoots(paths: readonly string[], home: string, cwd: string): void {
...
export function clearSanExtensionCliRoots(): void {
	injectedCliRoots = [];
}

/** Inspect currently-injected CLI roots (read-only). Exposed for diagnostics + tests. */
export function getInjectedSanExtensionCliRoots(): readonly SanExtensionRoot[] {
 ...
 * are dropped):
 *
 * 1. CLI roots injected via {@link injectSanExtensionCliRoots}
 ...
 */
export async function listSanExtensionRoots(ctx: LoadContext): Promise<SanExtensionRoot[]> {
