...
$r = Mci "open new type waveaudio alias san_rec"
if ($r -ne 0) { exit 1 }

Mci "set san_rec channels 1 samplespersec 16000 bitspersample 16"

$r = Mci "record san_rec"
if ($r -ne 0) {
    Mci "close san_rec"
    exit 1
}
...
async function startPowerShellRecording(outputPath: string): Promise<RecordingHandle> {
	...
	const scriptPath = path.join(os.tmpdir(), `san-stt-record-${Snowflake.next()}.ps1`);
...
export async function startRecording(outputPath: string): Promise<RecordingHandle> {
	...
	if (recorders.length === 0) {
		throw new Error("No audio recorder available — run `san setup speech`");
	}
	...
	throw new Error(`No audio recorder could start — run \`san setup speech\`.\n${failures.join("\n")}`);
}
