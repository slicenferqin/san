 ...
 *
 * Handles `san update` to check for and install updates.
 * Uses the installer that owns the active san executable when it can be detected.
 */
import * as fs from "node:fs";
import * as os from "node:os";
import * as path from "node:path";
import { pipeline } from "node:stream/promises";
import { $which, APP_NAME, isEnoent, VERSION } from "@san/utils";
import { $ } from "bun";
...
const PACKAGE = "@san/coding-agent";
const HOMEBREW_FORMULA = APP_NAME;
const MISE_TOOL = "github:slicenferqin/san";
/**
 * Official npm registry origin.
 *
 * Pinned across both the release metadata check and the bun install step so
 * they resolve the same published package version. A user's bun may point at
 * an unofficial mirror (corporate proxy, Taobao, etc.) that lags the official
 * registry, otherwise the install can reject the version the check selected.
 * See #1686.
 */
 ...
 */
const NATIVES_PACKAGE = "@san/natives";

/**
 * Platform tags the release pipeline publishes as
 * `@san/natives-<tag>` leaves. Mirrors `SUPPORTED_PLATFORMS` in
...
function isPathInDirectory(filePath: string, directoryPath: string): boolean {
	...
	// PATH-resolved san path can refer to the same directory through different
	...
	// links like Homebrew's `bin/san -> Cellar/.../bin/san`; the parent fallback
...
function resolveUpdateMethod(
	...
	const launcherExtension = path.extname(sanPath).toLowerCase();
	...
	if ((npmBinDir && isPathInDirectory(sanPath, npmBinDir)) || isWindowsScriptLauncher) return "npm";
	return "binary";
}
...
async function resolveUpdateTarget(): Promise<UpdateTarget> {
	...
	if (sanPath) {
		const method = resolveUpdateMethod(sanPath, bunBinDir, {
			homebrewPrefix,
			miseBinDirs,
			miseDataDir,
			npmBinDir,
...
/**
 * Get the latest published package version from the npm registry.
 * The San repository remains the source for binary release downloads.
 */
async function getLatestRelease(): Promise<ReleaseInfo> {
	...
	if (!version) {
		throw new Error("Failed to fetch release info: response missing version");
	}
	const tag = `v${version}`;
 ...
 * (`react@19.2.6@@@1`). Global San updates can leave one full copy per
...
async function pruneBunCacheAfterGlobalInstall(): Promise<BunInstallCachePruneResult | undefined> {
	...
	const packageNames = globalNodeModulesDir
		...
		: new Set<string>();
	const cacheName = path.basename(cacheDir).toLowerCase();
	if (packageNames.size === 0 && !cacheName.includes("san") && !cacheName.includes("omp")) return undefined;
 ...
 * `@san/natives-<tag>` leaf to `expectedVersion`. `bun install -g`
 ...
 * `@san/coding-agent` and the loader aborts at
...
export async function runUpdateCommand(opts: { force: boolean; check: boolean }): Promise<void> {
	...
	// Choose update method based on the prioritized san binary in PATH
	try {
