...
export async function runBenchCommand(command: BenchCommandArgs, deps: BenchDependencies = {}): Promise<BenchSummary> {
	...
	if (command.models.length === 0) {
		throw new Error("Pass at least one model selector, e.g. `san bench opus gpt-5.2`");
	}
	...
	try {
		...
		for (const { selector, model, thinking } of targets) {
			...
			// Preflight credentials before starting any runs. This preserves the old
			// sequential failure behavior without launching duplicate failures.
			...
			if (!preflightKey) {
				const failure: BenchRunFailure = {
					ok: false,
					error: `No credentials for provider "${model.provider}". Run \`san\` and use /login, or set the provider API key.`,
				};
			...
			// Launch up to `par` requests concurrently while printing results in index order.
			let nextToPrint = 0;
