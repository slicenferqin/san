 ...
 * `@san/utils/env`, which eagerly loads `.env` from the agent directory
 ...
 * doesn't look like another flag. Without this, `san --system-prompt --profile
 ...
 * `san --some-ext-flag --profile work` still selects a profile. Known value-less
 * launch flags ({@link VALUELESS_FLAGS}) are exempt so a trailing profile after
 * them also activates (`san --print --profile work`).
 */
...
export function extractProfileFlags(argv: readonly string[]): ProfileBootstrapResult {
	...
	for (let index = 0; index < argv.length; index += 1) {
		...
		// successor only when it is value-like — `san --bar val --profile work` keeps
		...
		// a trailing global flag still applies (`san --some-ext-bool --profile work`
		...
		// activates (`san --print --profile work`).
