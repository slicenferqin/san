/**
 * `san models` — list, search, and refresh available models.
 *
 * Subcommands:
 ...
 *   then list. This is the supported replacement for `rm -rf ~/.san/models.db`
 ...
 */
import type { Api, Effort, Model } from "@san/ai";
import { getSupportedEfforts } from "@san/catalog/model-thinking";
import { formatNumber, getProjectDir } from "@san/utils";
 ...
 * name doubles as a `san models <provider>` shortcut.
 */
...
/** `san models ls`/`find`: provider-grouped listing (one box table per provider). */
...
/**
 * Entry point for the standalone `san models` command: bootstraps auth storage,
...
export async function runModelsCommand(command: ModelsCommandArgs): Promise<void> {
	...
	if (action === "find" && (!pattern || pattern.trim().length === 0)) {
		process.stderr.write("`san models find` requires a search substring, e.g. `san models find minimax`\n");
		process.exitCode = 1;
		return;
	}
