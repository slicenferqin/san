...
function formatRun(
	...
	const lines = [
		...
		`Context plan refs: ${run.contextPlanRefs && run.contextPlanRefs.length > 0 ? run.contextPlanRefs.join(", ") : "none"}`,
		`Legacy context packet refs: ${run.contextPacketRefs.length > 0 ? run.contextPacketRefs.join(", ") : "none"}`,
