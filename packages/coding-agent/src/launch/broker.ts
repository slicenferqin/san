...
import { Process, type PtyRunResult, PtySession } from "@san/natives";
import { isEexist, isEnoent, logger, postmortem, procmgr, sanitizeText } from "@san/utils";
...
export async function startDaemonBrokerFromEnvironment(): Promise<void> {
	...
	if (!lease) return;
	process.title = "san daemon broker";
