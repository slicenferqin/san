...
import { getAutoresearchDbPath, getAutoresearchProjectDir, logger } from "@san/utils";
 ...
 *
 * Used to key per-project autoresearch state under the canonical San autoresearch directory.
...
async function resolveAutoresearchPaths(cwd: string): Promise<{ dbPath: string; projectDir: string }> {
	const override = process.env.SAN_AUTORESEARCH_DB_DIR ?? process.env.OMP_AUTORESEARCH_DB_DIR;
