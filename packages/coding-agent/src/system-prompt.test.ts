...
async function runProbeScenario(options: {
	...
	const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "san-gpu-probe-"));
	try {
		...
		await Bun.write(
			lspciPath,
			'#!/usr/bin/env sh\nprintf x >> "$SAN_GPU_PROBE_COUNT"\nif [ -n "$SAN_GPU_PROBE_VALID_OUTPUT" ]; then printf "%s\\n" "$SAN_GPU_PROBE_VALID_OUTPUT"; fi\nif [ "$SAN_GPU_PROBE_DESCENDANT_HOLDS_STDOUT" = "true" ]; then sleep "$SAN_GPU_PROBE_SLEEP" & exit 0; fi\nif [ "$SAN_GPU_PROBE_HOLD_STDOUT_OPEN" = "true" ]; then sleep "$SAN_GPU_PROBE_SLEEP" & wait "$!"; fi\nif [ -n "$SAN_GPU_PROBE_SLEEP" ]; then exec sleep "$SAN_GPU_PROBE_SLEEP"; fi\nexit 0\n',
		);
...
for (let index = 0; index < Number(process.env.SAN_GPU_PROBE_RUNS ?? "1"); index += 1) {
...
const countFile = Bun.file(process.env.SAN_GPU_PROBE_COUNT ?? "");
...
`,
		...
		const env: Record<string, string | undefined> = {
			...
			SAN_GPU_PROBE_COUNT: probeCountPath,
			SAN_GPU_PROBE_RUNS: String(options.runs),
		};
		...
		if (options.sleepSeconds === undefined) {
			delete env.SAN_GPU_PROBE_SLEEP;
		} else {
			env.SAN_GPU_PROBE_SLEEP = String(options.sleepSeconds);
		}
		if (options.holdStdoutOpen) {
			env.SAN_GPU_PROBE_HOLD_STDOUT_OPEN = "true";
		} else {
			delete env.SAN_GPU_PROBE_HOLD_STDOUT_OPEN;
		}
		if (options.descendantHoldsStdout) {
			env.SAN_GPU_PROBE_DESCENDANT_HOLDS_STDOUT = "true";
		} else {
			delete env.SAN_GPU_PROBE_DESCENDANT_HOLDS_STDOUT;
		}
		if (options.validOutput !== undefined) {
			env.SAN_GPU_PROBE_VALID_OUTPUT = options.validOutput;
		} else {
			delete env.SAN_GPU_PROBE_VALID_OUTPUT;
		}
