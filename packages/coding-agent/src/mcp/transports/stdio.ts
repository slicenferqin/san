...
import { getProjectDir, readJsonl, Snowflake } from "@san/utils";
...
export interface StdioSpawnCommand {
	 ...
	 *
	 * Windows uses this only when the San host has no console to share. When
	 ...
	 * wrapper chains must stay in the San console session so nested console
...
export async function resolveStdioSpawnCommand(
	...
	// same console session. Only hide the child when San itself has no console
