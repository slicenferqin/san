...
import type { OAuthCallbackFlowOptions } from "@san/ai/oauth/callback-server";
import { OAuthCallbackFlow } from "@san/ai/oauth/callback-server";
import type { OAuthController, OAuthCredentials } from "@san/ai/oauth/types";
import type { FetchImpl } from "@san/ai/types";
import { getActiveProfile } from "@san/utils/dirs";
import type { OAuthCredential } from "../session/auth-storage";

/** Credential-id prefix for San-managed MCP OAuth credentials keyed by profile and server URL. */
...
/**
 * Deterministic credential id for an MCP server URL scoped to a San profile.
 *
...
/** Whether a credential id was minted by San's MCP OAuth flows (either era). */
 ...
 *
 * Plane is stricter for San-synthesized fallback resources (e.g. using the
...
export class MCPOAuthFlow extends OAuthCallbackFlow {
	...
	async generateAuthUrl(state: string, redirectUri: string): Promise<{ url: string; instructions?: string }> {
		...
		if (existingResource) {
			...
			// provider-authored, not San's server-URL fallback. Preserve same-host
