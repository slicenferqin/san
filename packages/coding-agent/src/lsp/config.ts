...
import { $which, isRecord, logger, pathIsWithin, type WhichOptions } from "@san/utils";
...
function getConfigSources(cwd: string): ConfigSource[] {
	...
	// Project config directories (canonical .san/, then legacy .omp/.pi and third-party roots)
	...
	// User config directories (canonical ~/.san/agent/, then legacy and third-party roots)
 ...
 * 2. Project config dirs: .san/lsp.*, legacy .omp/.pi, and third-party config dirs (+ hidden variants)
 * 3. User config dirs: ~/.san/agent/lsp.*, legacy and third-party config dirs (+ hidden variants)
