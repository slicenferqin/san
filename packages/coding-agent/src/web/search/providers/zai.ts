 ...
 */
import { type ApiKey, type AuthStorage, type FetchImpl, getEnvApiKey, withAuth } from "@san/ai";
import { APP_NAME, isRecord, VERSION } from "@san/utils";
...
export async function searchZai(params: ZaiSearchParams): Promise<SearchResponse> {
	...
	const rawResult = await withAuth(keyOrResolver, key => callZaiSearch(key, params), {
		...
		missingKeyMessage: "Z.AI credentials not found. Set ZAI_API_KEY or login with 'san /login zai'.",
	});
