import { THINKING_EFFORTS } from "@san/ai";
...
export const DEFAULT_BASH_INTERCEPTOR_RULES: BashInterceptorRule[] = [
	...
	},
	{
		...
		tool: "hub",
		message:
			'Use the `hub` tool (`op:"start"`) for services, watchers, and debuggers so other San instances can observe and control them.',
	},
	{
		pattern:
...
export const SETTINGS_SCHEMA = {
	...
	// Auth broker — credentials proxied through a remote `san auth-broker serve`
	// host. Hidden from the UI; populate via env vars or hand-edited config.yml.
	// Env (`SAN_AUTH_BROKER_URL` / `SAN_AUTH_BROKER_TOKEN`) takes precedence so
	...
	"providers.maxInFlightRequests": {
		...
		ui: {
			...
			description:
				'Maximum concurrent LLM requests per provider id (for example "openai" or "anthropic"), shared across local San processes with this config root. Omitted providers are unlimited.',
		},
	},
		...
		ui: {
			...
			options: [
				...
				},
				{
					value: "project",
					label: "Per-project",
					description: "Save project role models in .san/config.yml; missing project roles use global defaults",
				},
			],
		},
	},
	...
	"startup.checkUpdate": {
		...
		ui: {
			...
			description: "Check for San updates on startup",
		},
	},
	...
	// and mint/enhance isolated managed skills under ~/.san/agent/managed-skills.
	...
	"san.contextSteady.digest.llm.modelRole": { type: "string", default: "@smol" },
	...
	"san.brain.enabled": {
		...
		ui: {
			...
			group: "Experimental",
			label: "San Brain",
			description: "Automatically govern durable Brain state and inject approved low-authority guidance.",
		},
	},
	"san.brain.mode": {
		type: "enum",
	...
	"san.brain.autoDecision.enabled": { type: "boolean", default: true },
	"san.brain.autoDecision.explicitMinConfidence": { type: "number", default: 0.75 },
	"san.brain.autoDecision.inferredMinConfidence": { type: "number", default: 0.85 },
	"san.brain.autoDecision.minIndependentEvidence": { type: "number", default: 2 },
	"san.brain.autoDecision.maxPerTurn": { type: "number", default: 10 },
	...
	"san.executionLoop.checks.projectDir": { type: "string", default: ".san/checks" },
	...
	"san.executionLoop.roles.oracle.enabledInModes": { type: "array", default: ["council"] as const },
	...
	"san.executionLoop.budget.soloMaxTurns": { type: "number", default: 3 },
	...
	"github.cache.enabled": {
		...
		ui: {
			...
			description: "Cache rendered issue/PR view output in ~/.san/cache/github-cache.db so repeated reads are free",
		},
	},
	...
	"worktree.base": {
		...
		ui: {
			...
			description:
				"Base directory for agent-managed worktrees — task-isolation copies, `github` PR checkouts, and `san worktree` cleanup all live here. Unset uses ~/.san/wt. Must be an absolute or ~-relative path; relative paths are ignored. The SAN_WORKTREE_DIR env var overrides this.",
		},
	},
	...
	"thinkingBudgets.ultra": { type: "number", default: 32768 },
} as const;
