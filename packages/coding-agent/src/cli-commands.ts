...
// commands. Without a guard `resolveCliArgv` rewrites e.g. `san marketplace add
// xyz` to `san launch marketplace add xyz`, silently forwarding the argv to the
...
// The real commands live under `san plugin <action>`; each entry maps a verb to
...
const RESERVED_TOP_LEVEL_WORDS: Record<string, string> = {
	...
	list: '`san list` is not a top-level command. Use `san plugin list` to list installed plugins, or run `san launch list` if you meant to send "list" as a prompt.',
	remove:
	...
	uninstall:
		'`san uninstall` is not a top-level command. Use `san plugin uninstall <name@marketplace>` to remove a plugin, or run `san launch uninstall` if you meant to send "uninstall" as a prompt.',
	marketplace:
		'`san marketplace` is not a top-level command. Use `san plugin marketplace <add|remove|update|list>` to manage marketplaces, or run `san launch marketplace` if you meant to send "marketplace" as a prompt.',
	discover:
		'`san discover` is not a top-level command. Use `san plugin discover [marketplace]` to browse available plugins, or run `san launch discover` if you meant to send "discover" as a prompt.',
	upgrade:
		'`san upgrade` is not a top-level command. Use `san plugin upgrade [name@marketplace]` to upgrade plugins, or run `san launch upgrade` if you meant to send "upgrade" as a prompt.',
	enable:
		'`san enable` is not a top-level command. Use `san plugin enable <name@marketplace>` to enable a plugin, or run `san launch enable` if you meant to send "enable" as a prompt.',
	disable:
...
export function resolveCliArgv(argv: string[]): ResolvedCliArgv {
	...
	// (`san --approval-mode=yolo acp`). `run` dispatches strictly on argv[0], so
