import { streamSimple } from "@san/ai";
import chalk from "chalk";
import { createDefaultBenchRuntime, resolveBenchTargets } from "../cli/bench-runtime";
...
export interface IfBenchDependencies {
	createRuntime?: typeof createDefaultBenchRuntime;
...
export async function runIfBenchCommand(
	...
	if (command.models.length === 0)
		throw new Error("Pass at least one model selector, e.g. `san if-bench opus gpt-5.2`");
