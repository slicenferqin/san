...
import { AsyncDrain, getAgentDbPath, getStatsDbPath, isRecord, logger } from "@san/utils";
...
/** Row shape read from a `san stats` messages table during backfill. */
 ...
 * Delegates auth credential operations to AuthCredentialStore from @san/ai.
...
`);
	 ...
	 * `san stats` database (`~/.san/stats.db`) into model_perf. Fire-and-forget:
	 ...
	 * A missing stats.db leaves the marker unset so a later `san stats` run
	...
	/**
	 * Imports recent measurable request rows from a `san stats` database
