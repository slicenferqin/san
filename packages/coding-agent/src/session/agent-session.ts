...
export class AgentSession {
	...
	async compact(customInstructions?: string, options?: CompactOptions): Promise<CompactionResult> {
		...
		try {
			...
			// summary. Configured snapcompact falls back to that summary when the
			// active model is text-only; an explicit `/compact snapcompact` keeps
			// its no-LLM contract and fails locally instead.
			const wantsSnapcompact =
				compactionPrep.kind !== "fromHook" &&
				effectiveSettings.strategy === "snapcompact" &&
				!customInstructions &&
				!options?.internalGuidance;
			const snapcompactModelIsTextOnly = wantsSnapcompact && !this.model.input.includes("image");
			const snapcompactReady = wantsSnapcompact && !snapcompactModelIsTextOnly;
			...
			if (snapcompactModelIsTextOnly) {
				if (compactMode?.name === "snapcompact") {
					this.emitNotice(
						"warning",
