#!/usr/bin/env bun
/**
 * Build the standalone share-viewer page the San relay serves at `GET /s/<id>`.
 *
 ...
 * hands the JSON to template.js via `window.__SAN_SESSION_DATA__`.
 *
...
// Pin the San web palette so the viewer is a public artifact matching the live
// collaboration client, not a per-user export that mirrors the host terminal theme.
const themeVars = await generateThemeVars("web");

const html = getTemplate()
	.replace("<theme-vars/>", () => `<style>:root { ${themeVars} }</style>`)
	.replace("<title>Session Export</title>", () => "<title>San session</title>")
...
if (!html.includes("__SAN_SESSION_DATA__")) throw new Error("share loader not injected");
