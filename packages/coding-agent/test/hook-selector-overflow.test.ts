...
import { HookSelectorComponent } from "@san/coding-agent/modes/components/hook-selector";
import { getThemeByName, setThemeInstance, theme } from "@san/coding-agent/modes/theme/theme";
		...
		const options = [
			{
				...
				description:
					"Authenticate via the provider keys and OAuth state already configured under ~/.san/agent without opening a new browser-based setup flow.",
			},
			{
				label: "Set up San in terminal",
				description:
