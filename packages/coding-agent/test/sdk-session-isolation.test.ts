...
import { getSessionsDir, removeSyncWithRetries, Snowflake } from "@san/utils";
...
describe("createAgentSession session storage isolation", () => {
	...
	it("loads obfuscator only when secrets exist", async () => {
		await withClearedSecretEnv(async () => {
			...
			fs.mkdirSync(path.join(cwd, ".san"), { recursive: true });
			fs.writeFileSync(path.join(cwd, ".san", "secrets.yml"), "- type: plain\n  content: sdk-secret-token-123456\n");
