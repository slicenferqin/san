...
import { createAcpConnection } from "@san/coding-agent/modes/acp/acp-mode";
...
describe("ACP lazy startup", () => {
	it("applies schema defaults for ACP background jobs and preserves explicit overrides", async () => {
		const { runRootCommand } = await import("@san/coding-agent/main");
...
`,
		...
		try {
			...
			const { runRootCommand } = await import("@san/coding-agent/main");
			const { createAgentSession } = await import("@san/coding-agent/sdk");
