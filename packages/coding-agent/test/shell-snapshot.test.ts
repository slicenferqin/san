...
import { getOrCreateSnapshot, sanitizeSnapshotForBrush } from "@san/coding-agent/utils/shell-snapshot";
...
describe("sanitizeSnapshotForBrush", () => {
	...
	it("leaves non-alias lines (functions, exports, unalias) untouched", () => {
		const snapshot = [
			"# Shell snapshot - generated by San agent",
			"unalias -a 2>/dev/null || true",
			"my_fn () { echo hi; }",
...
// `__san_emit_referenced_exports` (shell-snapshot-fn-env.sh) re-exports env
...
describe("getOrCreateSnapshot", () => {
	it("re-exports env vars referenced by snapshotted functions (issue #3470)", async () => {
		...
		// reason — UUID filenames shouldn't leak via `ls /tmp/san-shell-snapshots`.
	...
	it("cleans up the empty snapshot file when the shell exits with a non-zero code", async () => {
		...
		try {
			...
			const snapshotDir = path.join(testRoot, "san-shell-snapshots");
