...
describe("profile alias installer", () => {
	it("writes a bash-compatible function that forwards subcommands through san", async () => {
		...
		expect(result.command).toBe("san --profile=work");
		expect(files.get("/home/me/.bashrc")).toContain("san-work() {");
		expect(files.get("/home/me/.bashrc")).toContain('command san --profile=work "$@"');
	});
	...
	it("can target the current source invocation instead of the installed san binary", async () => {
		const files = new Map<string, string>();
		...
		expect(files.get("/Users/me/.zshrc")).toContain("san-work() {");
	...
	it("installs the zsh alias under ZDOTDIR when set", async () => {
		...
		expect(files.get(result.configPath)).toContain("san-work() {");
	});
	...
	it("writes a fish function that forwards argv", async () => {
		...
		const content = files.get("/Users/me/.config/fish/conf.d/san-profiles.fish") ?? "";
		expect(content).toContain("function san-work --wraps san");
		expect(content).toContain("command san --profile=work $argv");
	});
	...
	it("installs the fish alias under XDG_CONFIG_HOME when set", async () => {
		...
		expect(result.configPath).toBe("/home/me/.dotfiles/config/fish/conf.d/san-profiles.fish");
		expect(files.get(result.configPath)).toContain("function san-work --wraps san");
	});
	...
	it("writes a PowerShell function because aliases cannot carry arguments", async () => {
		...
		expect(content).toContain("function san-work");
		expect(content).toContain("& san --profile=work @args");
	});
	...
	it("detects pwsh from PSModulePath when SHELL is unset on Windows", async () => {
		...
		expect(files.get(result.configPath)).toContain("& san --profile=work @args");
	});
	...
	it("replaces a previous block for the same alias", async () => {
		const files = new Map<string, string>([
			[
				...
				[
					"before",
					"# >>> san profile alias: san-work >>>",
					"alias san-work='command san --profile=old'",
					"# <<< san profile alias: san-work <<<",
					"after",
				].join("\n"),
			],
		...
		expect(content).toContain('command san --profile=work "$@"');
	...
	it("refuses to rewrite a malformed managed block missing its end marker", async () => {
		...
		const original = ["# >>> san profile alias: san-work >>>", "san-work() {", "export SECRET=keepme"].join("\n");
	...
	it("refuses to shadow the base san command case-insensitively", async () => {
		for (const aliasName of ["san", "SAN"]) {
			await expect(
				installProfileAlias({
					profile: "work",
					aliasName,
	...
	it("normalizes backslashes in XDG_CONFIG_HOME for fish config paths on Windows", async () => {
		...
		expect(result.configPath).toBe("D:/xdg/fish/conf.d/san-profiles.fish");
		expect(result.reloadedWith).toBe("source 'D:/xdg/fish/conf.d/san-profiles.fish'");
	});
