/**
 * Discovery integration tests for the San plugin registry format and path.
 * End-to-end `listClaudePluginRoots` wiring is covered by project-scope.test.ts.
 */
...
// ── Inline validator ───────────────────────────────────────────────────────────
// Mirrors the registry shape consumed by `listClaudePluginRoots`.
...
// Canonical San project and user configuration directory.
const SAN_CONFIG_DIR = ".san";
...
let tmpHome: string;
/** ~/.san/plugins/installed_plugins.json inside tmpHome */
let sanRegistryPath: string;

beforeEach(() => {
	tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), "san-discovery-test-"));
	sanRegistryPath = path.join(tmpHome, SAN_CONFIG_DIR, "plugins", "installed_plugins.json");
	fs.mkdirSync(path.dirname(sanRegistryPath), { recursive: true });
});
...
describe("San registry path contract", () => {
	it("registry lives at home/.san/plugins/installed_plugins.json", () => {
		const expected = path.join(tmpHome, ".san", "plugins", "installed_plugins.json");
		expect(sanRegistryPath).toBe(expected);
	});
});

// ── Format compatibility ───────────────────────────────────────────────────────

describe("San registry format compatibility with Claude parser", () => {
	it("empty registry written by writeInstalledPluginsRegistry passes validator", async () => {
		await writeInstalledPluginsRegistry(sanRegistryPath, { version: 2, plugins: {} });

		const content = fs.readFileSync(sanRegistryPath, "utf8");
	...
	it("registry with installed plugin passes validator", async () => {
		...
		let reg = await readInstalledPluginsRegistry(sanRegistryPath);
		reg = addInstalledPlugin(reg, pluginId, entry);
		await writeInstalledPluginsRegistry(sanRegistryPath, reg);
...
describe("San registry round-trip", () => {
	it("reads back what was written — single plugin", async () => {
		...
		const readBack = await readInstalledPluginsRegistry(sanRegistryPath);
...
//
// `listClaudePluginRoots` replaces Claude entries with San entries when the
// same plugin ID appears in both registries.

describe("San precedence contract (registry structure)", () => {
	it("same plugin ID in both registries has required fields for deduplication", () => {
		const id = buildPluginId("shared-plugin", "common-mkt");
		const sanEntry = makeEntry("/san/cached/path");

		expect(sanEntry.installPath).toBeTruthy();
		expect(typeof sanEntry.installPath).toBe("string");
