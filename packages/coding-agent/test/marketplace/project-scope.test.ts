...
describe("resolveActiveProjectRegistryPath", () => {
	...
	it("walk-up finds nearest .san/ directory", async () => {
	...
	it("walk-up stops at the nearest .san/ — does not skip to a more distant one", async () => {
		// Layout: tmpDir/.san/   +   tmpDir/sub/.san/   +   tmpDir/sub/nested/  (cwd)
		// Resolver must stop at tmpDir/sub/.san/, not climb further to tmpDir/.san/.
		fs.mkdirSync(path.join(tmpDir, ".san"), { recursive: true });
		fs.mkdirSync(path.join(tmpDir, "sub", ".san"), { recursive: true });
		const cwd = path.join(tmpDir, "sub", "nested");
		fs.mkdirSync(cwd, { recursive: true });

		const result = await resolveActiveProjectRegistryPath(cwd);

		expect(result).toBe(path.join(tmpDir, "sub", ".san", "plugins", "installed_plugins.json"));
	});

	it("falls back to .git root when no .san/ exists", async () => {
		// Layout: tmpDir/.git/   +   tmpDir/sub/  (cwd)
		// No .san/ anywhere → second pass finds .git/ at tmpDir.
	...
	it("returns null when neither .san/ nor .git/ found anywhere in the tree", async () => {
		// Start at the filesystem root — guaranteed to have no .san/ or .git/ ancestors.
	...
	it("does not treat ~/.git as a project root (pass-2 home-dir guard)", async () => {
		...
		try {
			const result = await resolveActiveProjectRegistryPath(cwd);
			const homeSanPath = path.join(homeDir, ".san", "plugins", "installed_plugins.json");
			expect(result).not.toBe(homeSanPath);
			expect(result).toBeNull();
		} finally {
...
describe("listClaudePluginRoots — project shadows user", () => {
	...
	/** Path where listClaudePluginRoots reads the user San registry. */
	...
	beforeEach(() => {
		...
		userRegPath = path.join(tmpHome, ".san", "plugins", "installed_plugins.json");
		fs.mkdirSync(path.dirname(userRegPath), { recursive: true });

		projectRegPath = path.join(tmpProject, ".san", "plugins", "installed_plugins.json");
	});
