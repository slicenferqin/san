...
describe("advisor watchdog prompt discovery", () => {
	...
	it("discovers user-level and native project-level watchdog files", async () => {
		...
		tempDirs.push(tempDir);
		const cwd = tempDir.join("project-root");
		const sanDir = path.join(cwd, ".san");
		...
		fs.writeFileSync(path.join(sanDir, "WATCHDOG.md"), nativeWatchdogContent, "utf8");
		...
		try {
			...
			// Check ordering: user-level should appear first, then native project level (.san/WATCHDOG.md has depth 0),
