...
describe("legacy pi compat package-root override validation (issue #2168)", () => {
	it("keeps overrides whose filesystem targets exist", () => {
		const candidates = {
			"@san/ai": "/tmp/exists-ai.js",
			"@san/utils": "/tmp/exists-utils.js",
		};
	...
	it("drops overrides whose filesystem targets are missing on disk", () => {
		const candidates = {
			"@san/ai": "/tmp/exists-ai.js",
			"@san/coding-agent": "/tmp/exists-shim.js",
			"@san/utils": "/$bunfs/root/packages/utils/src/index.js",
			"@san/tui": "/$bunfs/root/packages/tui/src/index.js",
		};
		...
		expect(result).not.toHaveProperty("@san/utils");
		expect(result).not.toHaveProperty("@san/tui");
	});
	...
	it("keeps virtual omp-legacy-pi-bundled: entries without touching the filesystem (issue #3423)", () => {
		...
		let probed = false;
		const candidates = {
			"@san/ai": "omp-legacy-pi-bundled:@oh-my-pi/pi-ai",
			"@san/coding-agent": "omp-legacy-pi-bundled:@oh-my-pi/pi-coding-agent",
			"@san/agent": "omp-legacy-pi-bundled:@oh-my-pi/pi-agent-core",
			"@san/natives": "omp-legacy-pi-bundled:@oh-my-pi/pi-natives",
			"@san/tui": "omp-legacy-pi-bundled:@oh-my-pi/pi-tui",
			"@san/utils": "omp-legacy-pi-bundled:@oh-my-pi/pi-utils",
		};
	...
	it("mixes virtual and filesystem entries: virtuals always pass, filesystems gated", () => {
		const candidates = {
			"@san/ai": "omp-legacy-pi-bundled:@oh-my-pi/pi-ai",
			"@san/coding-agent": "/dev/source/legacy-pi-coding-agent-shim.ts",
			"@san/tui": "/missing/path.ts",
		};
