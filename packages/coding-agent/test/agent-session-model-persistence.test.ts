...
import { type Api, type AssistantMessage, Effort, type Model } from "@san/ai";
import { getBundledModel } from "@san/catalog/models";
import { ModelRegistry } from "@san/coding-agent/config/model-registry";
import { Settings } from "@san/coding-agent/config/settings";
import { type CreateAgentSessionResult, createAgentSession } from "@san/coding-agent/sdk";
...
import { getRestorableSessionModels } from "@san/coding-agent/session/session-context";
import { EPHEMERAL_MODEL_CHANGE_ROLE } from "@san/coding-agent/session/session-entries";
...
describe("AgentSession model persistence", () => {
	...
	it("marks an incomplete process-exit transcript aborted during SDK resume without dropping history", async () => {
		...
		expect(messages.at(-1)).toMatchObject({
			...
			content: [],
			stopReason: "aborted",
			errorMessage: "Previous San process exited before completing the turn.",
		});
		expect(
			messages.some(
				message =>
