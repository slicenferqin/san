...
import { initializeWithSettings, reset as resetDiscoveryCache } from "@san/coding-agent/discovery";
import { readMCPConfigFile, setMcpServerEnabled, setServerDisabled } from "@san/coding-agent/mcp/config-writer";
import { loadAllExtensions } from "@san/coding-agent/modes/components/extensions/state-manager";
import { __resetDirsFromEnvForTests, getMCPConfigPath, removeWithRetries, setAgentDir } from "@san/utils";
...
describe("loadAllExtensions MCP parity with /mcp list (issue #3827)", () => {
	...
	beforeEach(async () => {
		...
		await fs.writeFile(
			path.join(projectDir, ".san", "mcp.json"),
			JSON.stringify({
				mcpServers: {
	...
	test("dashboard re-enable flips enabled:false in mcp.json (PR #3829 review)", async () => {
		...
		const projectMcpPath = path.join(projectDir, ".san", "mcp.json");
	...
	test("dashboard disable on a config-resident server writes enabled:false (not denylist)", async () => {
		...
		const projectConfig = await readMCPConfigFile(path.join(projectDir, ".san", "mcp.json"));
	...
	test("dashboard re-enable updates the row's non-primary source mcp.json before denylisting", async () => {
		const alternatePath = path.join(projectDir, ".san", ".mcp.json");
		await Bun.write(
			alternatePath,
			JSON.stringify({
				mcpServers: {
