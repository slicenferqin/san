...
import { createReportBundle } from "@san/coding-agent/debug/report-bundle";
import { APP_NAME, getConfigRootDir, getLogsDir, removeWithRetries, setAgentDir } from "@san/utils";
...
describe("report bundle logs", () => {
	it("collects every same-day PID log, not only the current process", async () => {
		cleanupRoot = await fs.mkdtemp(path.join(os.tmpdir(), `${APP_NAME}-report-logs-`));
		const xdgStateHome = path.join(cleanupRoot, "state");
		await fs.mkdir(path.join(xdgStateHome, APP_NAME), { recursive: true });
