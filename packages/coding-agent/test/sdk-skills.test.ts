...
import { getActiveSkills } from "@san/coding-agent/extensibility/skills";
import type { Skill } from "@san/coding-agent/sdk";
...
describe("createAgentSession skills option", () => {
	...
	beforeEach(() => {
		...
		// Create skill in .san/skills/ for native project-level discovery
		skillsDir = path.join(tempDir, ".san", "skills", "test-skill");
...
`,
	...
	it("should still discover project skills when user skills directory is missing", async () => {
		const userAgentDir = path.join(tempHomeDir, ".san", "agent");
