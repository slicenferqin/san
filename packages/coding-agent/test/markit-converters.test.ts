...
import { convertBufferWithMarkit, convertFileWithMarkit } from "@san/coding-agent/utils/markit";
import { zip } from "@san/coding-agent/utils/zip";
...
describe("markit converters", () => {
	...
	it("reads PDF text after inline image binary data containing delimiter bytes", async () => {
		...
		const probe = [
			'import { convertBufferWithMarkit } from "@san/coding-agent/utils/markit";',
			"const bytes = await Bun.file(process.argv[1]!).bytes();",
			'const result = await convertBufferWithMarkit(bytes, ".pdf", undefined, { useCache: false });',
			'if (!result.ok) throw new Error(result.error ?? "PDF conversion failed");',
			"process.stdout.write(result.content);",
		].join("\n");
		const proc = Bun.spawn([process.execPath, "-e", probe, pdfPath], {
		...
		if (outcome.type === "timeout") {
			...
			await proc.exited;
			throw new Error("PDF conversion timed out on inline image data");
		}
