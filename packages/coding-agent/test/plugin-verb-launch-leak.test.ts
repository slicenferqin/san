 ...
 * `san list` / `san remove` / `san marketplace <sub>` / `san uninstall …` etc.
 ...
 * `san plugin list`). #4845 extended the same footgun to the multi-word
 * documented grammar: `san marketplace add xyz` leaked the whole argv to the
 * model as a prompt.
 *
 * These tests pin the chosen bugfix: a documented plugin/marketplace verb that
 * is bare, or that follows the documented grammar (a marketplace sub-action or
 * a `name@marketplace` plugin id), yields a helpful hint pointing at the real
 * `san plugin <action>` command rather than leaking to the model — while
...
describe("documented-but-unregistered plugin verbs do not leak to launch (#2935)", () => {
	...
	test("multi-word `omp marketplace add xyz` hints at `omp plugin marketplace` instead of leaking to the prompt (#4845)", () => {
		...
		expect(result).not.toHaveProperty("argv");
		expect(result).toHaveProperty("error");
		expect("error" in result && result.error).toContain("san plugin marketplace");
	});

	test("bare marketplace-family verbs hint at their `omp plugin` command (#4845)", () => {
		for (const [verb, hint] of [
			["marketplace", "san plugin marketplace"],
			["discover", "san plugin discover"],
			["upgrade", "san plugin upgrade"],
			["uninstall", "san plugin uninstall"],
			["enable", "san plugin enable"],
			["disable", "san plugin disable"],
		] as const) {
