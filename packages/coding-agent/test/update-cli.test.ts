...
import * as pluginCli from "@san/coding-agent/cli/plugin-cli";
import * as updateCli from "@san/coding-agent/cli/update-cli";
import {
	buildBunInstallArgs,
	...
	sweepStaleBackups,
} from "@san/coding-agent/cli/update-cli";
import Update from "@san/coding-agent/commands/update";
import { removeWithRetries } from "@san/utils";
import type { CliConfig } from "@san/utils/cli";

const tempDirs: string[] = [];

async function makeTempDir(): Promise<string> {
	const dir = await fs.mkdtemp(path.join(os.tmpdir(), "san-update-test-"));
	tempDirs.push(dir);
	return dir;
}
...
describe("update-cli install target detection", () => {
	it("uses bun update when prioritized san is inside bun global bin", () => {
		const method = resolveUpdateMethodForTest("/Users/test/.bun/bin/san", "/Users/test/.bun/bin");

		expect(method).toBe("bun");
	});

	it("uses npm update when prioritized san is inside an npm global bin", () => {
		const method = resolveUpdateMethodForTest("/Users/test/.npm-global/bin/san", undefined, {
	...
	it("uses npm update for Windows npm command shims even when no package-manager bin dirs were detected", () => {
		const method = resolveUpdateMethodForTest("C:\\Users\\test\\AppData\\Roaming\\npm\\san.cmd", undefined);

		expect(method).toBe("npm");
	});

	it("uses binary update when prioritized san is outside bun global bin", () => {
		const method = resolveUpdateMethodForTest("/Users/test/.local/bin/san", "/Users/test/.bun/bin");

		expect(method).toBe("binary");
	});

	it("uses binary update when bun global bin cannot be resolved", () => {
		const method = resolveUpdateMethodForTest("/Users/test/.local/bin/san", undefined);

		expect(method).toBe("binary");
	});

	it("uses Homebrew update when prioritized san resolves into the Homebrew formula", async () => {
		const dir = await makeTempDir();
		const prefix = path.join(dir, "opt", "san");
		const linkedBin = path.join(dir, "bin");
		await fs.mkdir(path.join(prefix, "bin"), { recursive: true });
		await fs.mkdir(linkedBin, { recursive: true });
		await Bun.write(path.join(prefix, "bin", "san"), "binary");
		await fs.symlink(path.join(prefix, "bin", "san"), path.join(linkedBin, "san"));

		const method = resolveUpdateMethodForTest(path.join(linkedBin, "san"), "/Users/test/.bun/bin", {
	...
	it("uses mise update when prioritized san is in an active mise bin path", () => {
		const method = resolveUpdateMethodForTest(
			"/Users/test/.local/share/mise/installs/github-slicenferqin-san/latest/bin/san",
			undefined,
			{
				miseBinDirs: ["/Users/test/.local/share/mise/installs/github-slicenferqin-san/latest/bin"],
			},
		);
	...
	it("uses mise update when prioritized san is a mise shim", () => {
		const method = resolveUpdateMethodForTest("/Users/test/.local/share/mise/shims/san", undefined, {
...
describe("update-cli package manager commands", () => {
	it("targets the Homebrew formula and switches to reinstall for forced updates", () => {
		expect(buildHomebrewUpdateArgs(false)).toEqual(["upgrade", "san"]);
		expect(buildHomebrewUpdateArgs(true)).toEqual(["reinstall", "san"]);
	});

	it("targets the mise GitHub backend tool and force-reinstalls the checked version when requested", () => {
		expect(buildMiseUpgradeArgs()).toEqual(["upgrade", "github:slicenferqin/san", "--bump"]);
		expect(buildMiseForceInstallArgs("15.10.5")).toEqual(["install", "--force", "github:slicenferqin/san@15.10.5"]);
	});
	...
	it("pins npm package installs to the official registry and the checked native package versions", () => {
		...
		expect(args).toContain("@san/coding-agent@16.3.15");
		expect(args).toContain("@san/natives@16.3.15");
		expect(args).toContain("@san/natives-win32-x64@16.3.15");
	});
});
...
describe("update-cli bun install command", () => {
	it("pins the official npm registry and bypasses the manifest cache so a stale mirror or snapshot cannot mask a freshly published version", () => {
		// The install MUST hit the official registry, otherwise:
	...
	it("pins the native addon core and the platform-specific leaf to the same version so the loader sentinel cannot drift on supported tags", () => {
		...
		// package, leaving @san/natives and @san/natives-<tag>
		...
		for (const tag of ["linux-x64", "linux-arm64", "darwin-x64", "darwin-arm64", "win32-x64"]) {
			...
			expect(args).toContain("@san/natives@15.9.0");
			expect(args).toContain(`@san/natives-${tag}@15.9.0`);
		}
	});
	...
	it("omits the leaf on unsupported platform tags so an EBADPLATFORM swap does not mask the underlying `no matching version` error", () => {
		...
		expect(args).toContain("@san/natives@15.9.0");
		expect(args.some(arg => arg.startsWith("@san/natives-"))).toBe(false);
	});
...
describe("update-cli bun cache pruning", () => {
	it("keeps only the newest cached version for filtered global install packages", async () => {
		...
		await Bun.write(path.join(dir, "@san", "utils", "15.7.6@@@1"), "");
		await Bun.write(path.join(dir, "@san", "utils", "15.8.0@@@1"), "");
		await Bun.write(
			path.join(dir, "@san", "utils@15.7.6@@@1", "package.json"),
			JSON.stringify({ name: "@san/utils", version: "15.7.6" }),
		);
		await Bun.write(
			path.join(dir, "@san", "utils@15.8.0@@@1", "package.json"),
			JSON.stringify({ name: "@san/utils", version: "15.8.0" }),
		);
		...
		const result = await pruneBunInstallCache(dir, new Set(["react", "@san/utils"]));
		...
		expect(await Bun.file(path.join(dir, "@san", "utils", "15.7.6@@@1")).exists()).toBe(false);
		expect(await Bun.file(path.join(dir, "@san", "utils@15.7.6@@@1", "package.json")).exists()).toBe(false);
		expect(await Bun.file(path.join(dir, "@san", "utils", "15.8.0@@@1")).exists()).toBe(true);
		expect(await Bun.file(path.join(dir, "@san", "utils@15.8.0@@@1", "package.json")).exists()).toBe(true);
...
describe("update-cli binary replacement", () => {
	it("restores the previous binary when the replacement fails verification", async () => {
		const dir = await makeTempDir();
		const targetPath = path.join(dir, "san");
		...
		).rejects.toThrow("restored previous san binary");
...
describe("update-cli binary replacement on locked backups", () => {
	it("treats an EPERM on backup cleanup as a successful, completed update", async () => {
		...
		const dir = await makeTempDir();
		const targetPath = path.join(dir, "san.exe");
		const tempPath = `${targetPath}.new`;
