 ...
 * `@san/utils` logger and metric instruments through the agent
 * telemetry hooks, flushes, and exits 0 only if the receiver got a non-empty
 * protobuf POST at both /v1/logs and /v1/metrics.
 */

import type { AgentRunCoverage, AgentRunSummary, ChatUsageEvent } from "@san/agent";
import { emptyAgentRunCoverage, emptyAgentRunSummary } from "@san/agent";
import {
	createTelemetryExportConfig,
	flushTelemetryExport,
	initTelemetryExport,
	isTelemetryExportEnabled,
} from "@san/coding-agent/telemetry-export";
...
process.env.OTEL_SERVICE_NAME = "san-signals-probe";
...
await Bun.sleep(700);
await flushTelemetryExport();
assertSingleMetricPoint("san.agent.chat.calls");
assertSingleMetricPoint("san.agent.tool.calls");
assertSingleMetricPoint("san.agent.tool.duration");
await server.stop(true);
