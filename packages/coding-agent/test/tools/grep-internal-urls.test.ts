...
import * as capability from "@san/coding-agent/capability";
import type { CapabilityResult } from "@san/coding-agent/capability/types";
import { Settings } from "@san/coding-agent/config/settings";
import { resetActiveSkillsForTests, setActiveSkills } from "@san/coding-agent/extensibility/skills";
import {
...
} from "@san/coding-agent/internal-urls";
import { AgentRegistry } from "@san/coding-agent/registry/agent-registry";
import * as sshFileTransfer from "@san/coding-agent/ssh/file-transfer";
...
describe("GrepTool internal URL resolution", () => {
	...
	it("rejects an RE2-unsupported pattern on a pure-virtual search (dialect parity)", async () => {
		...
		await expect(tool.execute("re2", { pattern: "(?<=alpha)line", path: "virtual://doc.md" })).rejects.toThrow(
			/Invalid regex/i,
		);
	});

	it("expands san:// root to grep embedded documentation files", async () => {
		const session = createSession();
		const tool = new GrepTool(session);
		...
		const text = getResultText(result);
		expect(text).toContain("# san://tools/grep.md");
		expect(text).toContain("Grep file contents with a regex across files");
	});

	it("expands omp://docs as a legacy documentation alias", async () => {
		const session = createSession();
		const tool = new GrepTool(session);
