/**
 * Regression for the Windows `bun install -g` update path: when a San
 * process is running, bun cannot overwrite a locked
 * `node_modules/@san/natives/native/pi_natives.win32-x64.node` during
 ...
 *      (per-package-version cache under `~/.san/natives/<version>/`) so the
...
const winWorkspaceNativeDir = "C:\\Users\\Admin\\dev\\san\\packages\\natives\\native";
const posixNodeModulesNativeDir = "/home/u/proj/node_modules/@san/natives/native";
...
describe("windows native addon staging", () => {
	...
	it("prepends versionedDir candidates ahead of node_modules when staging on Windows", () => {
		const versionedDir = "C:\\Users\\Admin\\.san\\natives\\15.0.1";
		const userDataDir = "C:\\Users\\Admin\\AppData\\Local\\san";
	...
	it("falls back to the node_modules-only candidate list when staging is off", () => {
		...
		const versionedDir = "/home/u/.san/natives/15.0.1";
		const candidates = resolveLoaderCandidates({
	...
	it("removes stale version directories after the current native version loads", async () => {
		const nativesDir = await fs.mkdtemp(path.join(os.tmpdir(), "san-natives-cache-"));
		try {
