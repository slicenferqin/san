...
smoke_cli() {
   ...
   XDG_DATA_HOME="$runtime_dir/xdg" HOME="$runtime_dir/home" "$san_bin" --version
   XDG_DATA_HOME="$runtime_dir/xdg" HOME="$runtime_dir/home" "$san_bin" --help >/dev/null
   XDG_DATA_HOME="$runtime_dir/xdg" HOME="$runtime_dir/home" "$san_bin" stats --summary >/dev/null
   ...
   XDG_DATA_HOME="$runtime_dir/xdg" HOME="$runtime_dir/home" "$san_bin" --smoke-test
}
...
mkdir -p "$BINARY_DIR"
cp packages/coding-agent/dist/san "$BINARY_DIR/san"
smoke_cli "$BINARY_DIR/san"
...
#    `bin.san` from `src/cli.ts` to the prepack bundle `dist/cli.js`. The repo
...
utils_tgz="$(find_tarball "$TARBALL_DIR"/san-utils-*.tgz)"
wire_tgz="$(find_tarball "$TARBALL_DIR"/san-wire-*.tgz)"
natives_tgz="$(find_tarball "$TARBALL_DIR"/san-natives-[0-9]*.tgz)"
natives_leaf_tgz="$(find_tarball "$TARBALL_DIR"/san-natives-"$host_tag"-*.tgz)"
hashline_tgz="$(find_tarball "$TARBALL_DIR"/san-hashline-*.tgz)"
catalog_tgz="$(find_tarball "$TARBALL_DIR"/san-catalog-*.tgz)"
ai_tgz="$(find_tarball "$TARBALL_DIR"/san-ai-*.tgz)"
mnemopi_tgz="$(find_tarball "$TARBALL_DIR"/san-mnemopi-*.tgz)"
snapcompact_tgz="$(find_tarball "$TARBALL_DIR"/san-snapcompact-*.tgz)"
agent_tgz="$(find_tarball "$TARBALL_DIR"/san-agent-*.tgz)"
tui_tgz="$(find_tarball "$TARBALL_DIR"/san-tui-*.tgz)"
stats_tgz="$(find_tarball "$TARBALL_DIR"/san-stats-*.tgz)"
coding_agent_tgz="$(find_tarball "$TARBALL_DIR"/san-coding-agent-*.tgz)"
collab_web_tgz="$(find_tarball "$TARBALL_DIR"/san-collab-web-*.tgz)"
...
(
   ...
   node -e "
		...
		pkg.overrides = {
			'@san/utils': '$utils_tgz',
			'@san/wire': '$wire_tgz',
			'@san/natives': '$natives_tgz',
			'@san/natives-$host_tag': '$natives_leaf_tgz',
			'@san/hashline': '$hashline_tgz',
			'@san/ai': '$ai_tgz',
			'@san/catalog': '$catalog_tgz',
			'@san/mnemopi': '$mnemopi_tgz',
			'@san/snapcompact': '$snapcompact_tgz',
			'@san/agent': '$agent_tgz',
			'@san/tui': '$tui_tgz',
			'@san/stats': '$stats_tgz',
			'@san/coding-agent': '$coding_agent_tgz',
			'@san/collab-web': '$collab_web_tgz'
		};
   ...
   leaf_dir="node_modules/@san/natives-$host_tag"
   [ -d "$leaf_dir" ] || {
      echo "Platform leaf package not installed: $leaf_dir"
      exit 1
   }
   wire_proto="$(bun -e 'import { COLLAB_PROTO } from "@san/wire"; process.stdout.write(String(COLLAB_PROTO));')"
   [ "$wire_proto" = "3" ] || {
      echo "Unexpected @san/wire COLLAB_PROTO: $wire_proto"
      exit 1
   }
   [ -f "node_modules/@san/collab-web/dist/index.html" ] || {
      echo "Collab web tarball did not install built dist/index.html"
      exit 1
   }
   smoke_cli ./node_modules/.bin/san
)
