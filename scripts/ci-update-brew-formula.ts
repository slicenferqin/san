#!/usr/bin/env bun
//
// Render the Homebrew formula for `san` from a published GitHub release and write
// it to a tap checkout. The release publishes per-platform bare binaries
// (san-<platform>-<arch>); this reads their sha256 digests straight from the
// release metadata so the formula never drifts from the shipped assets.
//
// Usage:
//   bun scripts/ci-update-brew-formula.ts <tag> --out <path/to/Formula/san.rb>
//   bun scripts/ci-update-brew-formula.ts v15.10.3        # prints to stdout

import { $ } from "bun";

const REPO = process.env.SAN_REPO ?? process.env.OMP_REPO ?? "slicenferqin/san";
const HOMEPAGE = "https://github.com/slicenferqin/san";
...
export function renderFormula(version: string, sums: Record<string, string>): string {
	...
	// which nests the file outside the staging CWD; `Dir["san-*"].first` then
	// returns `nil` and `bin.install nil => "san"` raises.
	//
	...
	// not touch the real `/Users/<user>/.san` (denied by Homebrew's sandbox
	// profile, which would otherwise fail the popen).
	return `class San < Formula
  desc "${DESC}"
  homepage "${HOMEPAGE}"
  version "${version}"
  ...
  on_macos do
    on_arm do
      url "https://github.com/${REPO}/releases/download/v#{version}/san-darwin-arm64",
          using: :nounzip
      sha256 "${sums["san-darwin-arm64"]}"
    end
    on_intel do
      url "https://github.com/${REPO}/releases/download/v#{version}/san-darwin-x64",
          using: :nounzip
      sha256 "${sums["san-darwin-x64"]}"
    end
  end

  on_linux do
    on_arm do
      url "https://github.com/${REPO}/releases/download/v#{version}/san-linux-arm64",
          using: :nounzip
      sha256 "${sums["san-linux-arm64"]}"
    end
    on_intel do
      url "https://github.com/${REPO}/releases/download/v#{version}/san-linux-x64",
          using: :nounzip
      sha256 "${sums["san-linux-x64"]}"
    end
  end

  def install
    bin.install Dir["san-*"].first => "san"
    (bin/"san").chmod 0555
    with_env(HOME: buildpath) do
      generate_completions_from_executable(bin/"san", "completions", shells: [:bash, :zsh, :fish])
    end
  end

  test do
    assert_match version.to_s, shell_output("#{bin}/san --version")
  end
end
`;
}
...
async function main(): Promise<void> {
	...
	const targets = ["san-darwin-arm64", "san-darwin-x64", "san-linux-arm64", "san-linux-x64"];
