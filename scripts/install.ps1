# San Coding Agent Installer for Windows
# Usage: irm https://raw.githubusercontent.com/slicenferqin/san/main/scripts/install.ps1 | iex
#
# Or with options:
#   & ([scriptblock]::Create((irm https://raw.githubusercontent.com/slicenferqin/san/main/scripts/install.ps1))) -Source
#   & ([scriptblock]::Create((irm https://raw.githubusercontent.com/slicenferqin/san/main/scripts/install.ps1))) -Binary
#   & ([scriptblock]::Create((irm https://raw.githubusercontent.com/slicenferqin/san/main/scripts/install.ps1))) -Source -Ref v3.20.1
#   & ([scriptblock]::Create((irm https://raw.githubusercontent.com/slicenferqin/san/main/scripts/install.ps1))) -Source -Ref main
#   & ([scriptblock]::Create((irm https://raw.githubusercontent.com/slicenferqin/san/main/scripts/install.ps1))) -Binary -Ref v3.20.1
...
function Configure-BashShell {
    try {
        $settingsDir = Join-Path $env:USERPROFILE ".san\agent"
        ...
        if ($bashPath) {
            ...
            Write-Host "  San requires a bash shell on Windows. Options:" -ForegroundColor Yellow
...
function Install-ViaBun {
    ...
    if ($Ref) {
        ...
        $tmpRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("san-install-" + [System.Guid]::NewGuid().ToString("N"))
    ...
    Write-Host ""
    Write-Host "✓ Installed san via bun" -ForegroundColor Green

    Configure-BashShell

    Write-Host "Run 'san' to get started!"
}
...
function Install-Binary {
    ...
    $OutPath = Join-Path $InstallDir "san.exe"
    Invoke-WebRequest -Uri $BinaryUrl -OutFile $OutPath

    Write-Host ""
    Write-Host "✓ Installed san to $OutPath" -ForegroundColor Green
    ...
    if ($needsRestart) {
        Write-Host "Restart your terminal, then run 'san' to get started!"
    } else {
        Write-Host "Run 'san' to get started!"
    }
}
