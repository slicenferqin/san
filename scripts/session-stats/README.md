...
(`~/.san/agent/sessions/`). SQLite-backed; data is synced once into the same
`~/.san/stats.db` that `packages/stats` uses, then queried by short Python
scripts.
...
scripts/session-stats/
  sync.py          # walks ~/.san/agent/sessions/ and populates ss_* tables
...
The LLM phase (default `anthropic/claude-sonnet-4-6` via `@san/ai`,
credentials resolved through San's auth storage — stored key, OAuth, or env
...
Verdicts are cached in `~/.san/stats-audit-cache.json` (keyed by session id +
